 ...
 * Provides comprehensive data access methods for inventory management including
 * stock availability, reservations, sales tracking, and cleanup operations.
 * 
 * รีพอสิทอรี่สำหรับจัดการข้อมูลสต็อก
 * รองรับการจัดการสต็อก การจอง การติดตามการขาย และการล้างข้อมูล
 */
@Repository
public interface StockRepository extends JpaRepository<Stock, UUID> {
    ...
    /**
     * Find stock items by product.
     * Used for viewing all stock items for a specific product.
     * 
     ...
     * Used for product stock management.
     * 
    ...
    /**
     * Find stock items by product with pagination.
     * Used for paginated stock listing in admin interfaces.
     * 
     * @param product the product to search stock for
    ...
    /**
     * Find available stock items (not sold and not reserved).
     * Used for identifying stock that can be sold immediately.
     * 
     * @return list of available stock items
     */
    @Query("SELECT s FROM Stock s WHERE s.sold = false AND (s.reservedUntil IS NULL OR s.reservedUntil < CURRENT_TIMESTAMP) ORDER BY s.createdAt ASC")
    List<Stock> findAvailableStock();

    /**
     * Find available stock items by product.
     * Used for product-specific stock availability checks.
     * 
     * @param product the product to search available stock for
     * @return list of available stock items for the product
     */
    @Query("SELECT s FROM Stock s WHERE s.product = :product AND s.sold = false AND (s.reservedUntil IS NULL OR s.reservedUntil < CURRENT_TIMESTAMP) ORDER BY s.createdAt ASC")
    List<Stock> findAvailableStockByProduct(@Param("product") Product product);

    /**
     * Find available stock items by product ID.
     * Used for product-specific stock availability checks.
     * 
     * @param productId the product ID to search available stock for
     * @return list of available stock items for the product
     */
    @Query("SELECT s FROM Stock s WHERE s.product.id = :productId AND s.sold = false AND (s.reservedUntil IS NULL OR s.reservedUntil < CURRENT_TIMESTAMP) ORDER BY s.createdAt ASC")
    List<Stock> findAvailableStockByProductId(@Param("productId") UUID productId);

    /**
     * Find first available stock item by product.
     * Used for immediate stock allocation during purchase.
     * 
     * @param product the product to find available stock for
     * @return first available stock item or empty optional
     */
    @Query("SELECT s FROM Stock s WHERE s.product = :product AND s.sold = false AND (s.reservedUntil IS NULL OR s.reservedUntil < CURRENT_TIMESTAMP) ORDER BY s.createdAt ASC LIMIT 1")
    Optional<Stock> findFirstAvailableByProduct(@Param("product") Product product);

    /**
     * Find first available stock item by product ID.
     * Used for immediate stock allocation during purchase.
     * 
     * @param productId the product ID to find available stock for
     * @return first available stock item or empty optional
     */
    @Query("SELECT s FROM Stock s WHERE s.product.id = :productId AND s.sold = false AND (s.reservedUntil IS NULL OR s.reservedUntil < CURRENT_TIMESTAMP) ORDER BY s.createdAt ASC LIMIT 1")
    Optional<Stock> findFirstAvailableByProductId(@Param("productId") UUID productId);

    // ==================== SOLD STOCK QUERIES ====================

    /**
     * Find sold stock items.
     * Used for sales tracking and reporting.
     * 
    ...
    /**
     * Find sold stock items by product.
     * Used for product-specific sales tracking.
     * 
    ...
    /**
     * Find sold stock items within date range.
     ...
     */
    @Query("SELECT s FROM Stock s WHERE s.sold = true AND s.soldAt >= :startDate AND s.soldAt <= :endDate ORDER BY s.soldAt DESC")
    List<Stock> findSoldBetweenDates(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    /**
     * Find sold stock items by product within date range.
     * Used for product-specific sales reporting.
     * 
     * @param product the product to search sold stock for
     * @param startDate start date of the range
     * @param endDate end date of the range
     ...
     */
    @Query("SELECT s FROM Stock s WHERE s.product = :product AND s.sold = true AND s.soldAt >= :startDate AND s.soldAt <= :endDate ORDER BY s.soldAt DESC")
    List<Stock> findSoldByProductBetweenDates(@Param("product") Product product, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    // ==================== RESERVATION QUERIES ====================

    /**
     * Find reserved stock items.
     * Used for tracking current reservations.
     * 
     * @return list of currently reserved stock items
     */
    @Query("SELECT s FROM Stock s WHERE s.sold = false AND s.reservedUntil IS NOT NULL AND s.reservedUntil > CURRENT_TIMESTAMP ORDER BY s.reservedUntil ASC")
    List<Stock> findReservedStock();

    /**
     * Find reserved stock items by product.
     * Used for product-specific reservation tracking.
     * 
     * @param product the product to search reserved stock for
     * @return list of reserved stock items for the product
     */
    @Query("SELECT s FROM Stock s WHERE s.product = :product AND s.sold = false AND s.reservedUntil IS NOT NULL AND s.reservedUntil > CURRENT_TIMESTAMP ORDER BY s.reservedUntil ASC")
    List<Stock> findReservedStockByProduct(@Param("product") Product product);

    /**
     * Find expired reservation stock items.
     * Used for cleanup operations and releasing expired reservations.
     * 
     * @return list of stock items with expired reservations
     */
    @Query("SELECT s FROM Stock s WHERE s.sold = false AND s.reservedUntil IS NOT NULL AND s.reservedUntil <= CURRENT_TIMESTAMP")
    List<Stock> findExpiredReservations();

    /**
     * Find reservations expiring soon.
     * Used for proactive reservation management and notifications.
     * 
     ...
     */
    @Query("SELECT s FROM Stock s WHERE s.sold = false AND s.reservedUntil IS NOT NULL AND s.reservedUntil > CURRENT_TIMESTAMP AND s.reservedUntil <= :threshold ORDER BY s.reservedUntil ASC")
    List<Stock> findReservationsExpiringSoon(@Param("threshold") LocalDateTime threshold);

    // ==================== STOCK COUNTING QUERIES ====================

    /**
     * Count total stock items by product.
    ...
    /**
     * Count available stock items by product.
     ...
     */
    @Query("SELECT COUNT(s) FROM Stock s WHERE s.product = :product AND s.sold = false AND (s.reservedUntil IS NULL OR s.reservedUntil < CURRENT_TIMESTAMP)")
    long countAvailableByProduct(@Param("product") Product product);

    /**
     * Count available stock items by product ID.
     * Used for stock availability checks.
     * 
     * @param productId the product ID to count available stock for
     * @return count of available stock items for the product
     */
    @Query("SELECT COUNT(s) FROM Stock s WHERE s.product.id = :productId AND s.sold = false AND (s.reservedUntil IS NULL OR s.reservedUntil < CURRENT_TIMESTAMP)")
    long countAvailableByProductId(@Param("productId") UUID productId);

    /**
     * Count sold stock items by product.
    ...
    /**
     * Count reserved stock items by product.
     * Used for reservation reporting.
     * 
     * @param product the product to count reserved stock for
     * @return count of reserved stock items for the product
     */
    @Query("SELECT COUNT(s) FROM Stock s WHERE s.product = :product AND s.sold = false AND s.reservedUntil IS NOT NULL AND s.reservedUntil > CURRENT_TIMESTAMP")
    long countReservedByProduct(@Param("product") Product product);

    // ==================== BULK OPERATIONS ====================

    /**
     * Clear expired reservations in bulk.
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Stock s SET s.reservedUntil = NULL WHERE s.sold = false AND s.reservedUntil IS NOT NULL AND s.reservedUntil <= CURRENT_TIMESTAMP")
    int clearExpiredReservations();

    /**
     * Reserve multiple stock items for a product.
     * Used for bulk reservation operations.
     * 
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    int reserveStockItems(@Param("productId") UUID productId, @Param("reservedUntil") LocalDateTime reservedUntil, @Param("limit") int limit);

    /**
     * Release reservations for specific stock items.
     * Used for canceling reservations.
     * 
    ...
    /**
     * Find stock items with low availability by category.
     ...
     * Used for debugging and administrative searches.
    ...
    /**
     * Find stock statistics grouped by product.
     * Used for comprehensive inventory reporting.
     * 
    ...
    /**
     * Delete sold stock items older than specified date.
    ...
    /**
     * Find duplicate credentials within the same product.
    ...
    /**
     * Check if stock item exists with specific credentials for a product.
     * Used for preventing duplicate stock entries.
     * 
    ...
    /**
     * Find the oldest unsold stock item by product.
     * Used for FIFO (First In, First Out) stock allocation.
     * 
     * @param product the product to find oldest stock for
     * @return oldest unsold stock item or empty optional
     */
    @Query("SELECT s FROM Stock s WHERE s.product = :product AND s.sold = false ORDER BY s.createdAt ASC LIMIT 1")
    Optional<Stock> findOldestUnsoldByProduct(@Param("product") Product product);

    /**
     * Find stock items created within date range.
     * Used for tracking stock additions and inventory analysis.
     * 
     * @param startDate start date of the range
     * @param endDate end date of the range
     * @return list of stock items created within the date range
     */
    @Query("SELECT s FROM Stock s WHERE s.createdAt >= :startDate AND s.createdAt <= :endDate ORDER BY s.createdAt DESC")
    List<Stock> findStockCreatedBetweenDates(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
}