 ...
 * Provides comprehensive data access methods for order item management including
 * order details, product sales tracking, stock item relationships, and analytics.
 * 
 * รีพอสิทอรี่สำหรับจัดการข้อมูลรายการสินค้าในคำสั่งซื้อ
 * รองรับการจัดการรายละเอียดคำสั่งซื้อ การติดตามการขายสินค้า ความสัมพันธ์กับสต็อก และการวิเคราะห์
 */
@Repository
public interface OrderItemRepository extends JpaRepository<OrderItem, UUID> {
     ...
     * Used for displaying order details and item breakdown.
     * 
     ...
     * Used for paginated order item display.
     * 
     * @param order the order to search items for
     ...
     * Used for order item management by order ID.
     * 
    ...
    /**
     * Find order item by stock item.
     * Used for tracking which order item contains a specific stock item.
     * 
    ...
    /**
     * Find order item by stock item ID.
     * Used for stock item to order item mapping.
     * 
     ...
     * Used for product sales tracking and analytics.
     * 
     ...
     * Used for paginated product sales tracking.
     * 
     * @param product the product to search order items for
     * @param pageable pagination parameters
     ...
     * Used for product-specific sales analysis.
     * 
    ...
    /**
     * Find order items by product within date range.
     * Used for product sales reporting within specific periods.
     * 
     * @param product the product to search order items for
     ...
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.product = :product AND oi.createdAt >= :startDate AND oi.createdAt <= :endDate ORDER BY oi.createdAt DESC")
    List<OrderItem> findByProductBetweenDates(@Param("product") Product product, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    // ==================== USER-BASED QUERIES ====================

    /**
     * Find order items by user.
     * Used for user purchase history and analytics.
     * 
    ...
    /**
     * Find order items by user with pagination.
     * Used for paginated user purchase history.
     * 
     * @param user the user to search order items for
     * @param pageable pagination parameters
     ...
     * Used for user-specific purchase tracking.
     * 
    ...
    /**
     * Find order items by user and product.
     * Used for tracking user purchases of specific products.
     * 
     * @param user the user to search order items for
     * @param product the product to filter by
     * @return list of order items for the user and product
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.order.user = :user AND oi.product = :product ORDER BY oi.createdAt DESC")
     ...
     * Used for status-based order item filtering.
     * 
    ...
    /**
     * Find order items by order status with pagination.
     * Used for paginated status-based order item filtering.
     * 
     * @param status the order status to filter by
     * @param pageable pagination parameters
     ...
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.order.status = 'COMPLETED' ORDER BY oi.createdAt DESC")
    List<OrderItem> findCompletedOrderItems();

    /**
     * Find completed order items by product.
     * Used for product-specific sales analytics.
     * 
     * @param product the product to search completed order items for
     * @return list of completed order items for the product
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.product = :product AND oi.order.status = 'COMPLETED' ORDER BY oi.createdAt DESC")
    List<OrderItem> findCompletedOrderItemsByProduct(@Param("product") Product product);

    // ==================== DATE-BASED QUERIES ====================

    /**
     * Find order items created within date range.
     ...
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.createdAt >= :startDate AND oi.createdAt <= :endDate ORDER BY oi.createdAt DESC")
    List<OrderItem> findOrderItemsBetweenDates(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    /**
     * Find completed order items within date range.
     * Used for sales reporting within specific periods.
     * 
     * @param startDate start date of the range
     * @param endDate end date of the range
     * @return list of completed order items within the date range
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.order.status = 'COMPLETED' AND oi.createdAt >= :startDate AND oi.createdAt <= :endDate ORDER BY oi.createdAt DESC")
    List<OrderItem> findCompletedOrderItemsBetweenDates(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    // ==================== PRICE-BASED QUERIES ====================

    /**
     * Find order items by price range.
     * Used for price-based analysis and filtering.
     * 
     * @param minPrice minimum price (inclusive)
     * @param maxPrice maximum price (inclusive)
     * @return list of order items within the specified price range
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.price >= :minPrice AND oi.price <= :maxPrice ORDER BY oi.price DESC")
    List<OrderItem> findByPriceRange(@Param("minPrice") BigDecimal minPrice, @Param("maxPrice") BigDecimal maxPrice);

    /**
     * Find order items above specified price.
     * Used for high-value item tracking.
     * 
     ...
     * Used for order item count statistics.
     * 
     ...
     * Used for product sales count statistics.
     * 
    ...
    /**
     * Count completed order items by product.
     * Used for product sales statistics.
     * 
     * @param product the product to count completed order items for
     * @return count of completed order items for the product
     */
    @Query("SELECT COUNT(oi) FROM OrderItem oi WHERE oi.product = :product AND oi.order.status = 'COMPLETED'")
    long countCompletedByProduct(@Param("product") Product product);

    /**
     * Count order items by user.
     * Used for user purchase statistics.
     * 
    ...
    /**
     * Count order items by order status.
     ...
     * Used for product-specific revenue analysis.
     * 
     * @param product the product to calculate revenue for
     * @return total revenue from the product's completed order items
     */
    @Query("SELECT COALESCE(SUM(oi.price), 0) FROM OrderItem oi WHERE oi.product = :product AND oi.order.status = 'COMPLETED'")
     ...
     */
    @Query("SELECT COALESCE(SUM(oi.price), 0) FROM OrderItem oi WHERE oi.order.status = 'COMPLETED' AND oi.createdAt >= :startDate AND oi.createdAt <= :endDate")
    BigDecimal calculateRevenueBetweenDates(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    /**
     * Calculate average item price.
     * Used for pricing analytics.
     * 
     ...
     */
    @Query("SELECT oi.product FROM OrderItem oi WHERE oi.order.status = 'COMPLETED' GROUP BY oi.product ORDER BY COUNT(oi) DESC LIMIT :limit")
    List<Product> findTopSellingProductsByQuantity(@Param("limit") int limit);

    /**
     * Find top-selling products by revenue.
     * Used for revenue-based product analysis.
     * 
     * @param limit maximum number of products to return
     * @return list of products ordered by total revenue
     */
    @Query("SELECT oi.product FROM OrderItem oi WHERE oi.order.status = 'COMPLETED' GROUP BY oi.product ORDER BY SUM(oi.price) DESC LIMIT :limit")
    List<Product> findTopSellingProductsByRevenue(@Param("limit") int limit);

    /**
     * Find recent order items for dashboard.
     ...
     * Used for category-based sales analysis.
     * 
     * @param categoryName the category name to filter by
     * @return list of order items for products in the specified category
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.product.category.name = :categoryName ORDER BY oi.createdAt DESC")
    List<OrderItem> findByCategory(@Param("categoryName") String categoryName);

    /**
     * Find completed order items by category.
     * Used for category-based sales reporting.
     * 
     * @param categoryName the category name to filter by
     * @return list of completed order items for products in the specified category
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.product.category.name = :categoryName AND oi.order.status = 'COMPLETED' ORDER BY oi.createdAt DESC")
    List<OrderItem> findCompletedByCategory(@Param("categoryName") String categoryName);

    /**
     * Search order items by multiple criteria.
     * Used for advanced order item search with multiple filters.
     * 
     ...
     * Used for comprehensive product performance analysis.
     * 
    ...
    /**
     * Get daily sales statistics within date range.
     * Used for daily sales reporting and charts.
     * 
     * @param startDate start date of the range
     * @param endDate end date of the range
    ...
    @Query("""
        ...
        """)
    List<Object[]> getDailySalesStatistics(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    /**
     * Check if stock item is already used in an order item.
     * Used for preventing duplicate stock item usage.
     * 
    ...
    /**
     * Find order items with invalid stock relationships.
    ...
    /**
     * Find order items by category name.
    ...
    /**
     * Find order items by server.
     * Used for server-based filtering.
     * 
    ...
    /**
     * Find order items by username.
     * Used for user-based search.
     * 
     * @param username the username
     * @return list of order items for the specified user
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.order.user.username = :username ORDER BY oi.createdAt DESC")
    ...
    /**
     * Find order items by product and created between dates.
     * Used for product-specific date range analysis.
     * 
     ...
     */
    List<OrderItem> findByProductAndCreatedAtBetween(Product product, LocalDateTime startDate, LocalDateTime endDate);

    /**
     * Find order items by price between range.
    ...
    /**
     * Find order items by price greater than.
     * Used for minimum price filtering.
     * 
    ...
    /**
     * Calculate total revenue by product.
     * Used for product revenue analysis.
     * 
     * @param product the product
     ...
     * Used for user spending analysis.
     * 
     * @param user the user
     * @return total amount spent by the user
     */
    @Query("SELECT COALESCE(SUM(oi.price), 0) FROM OrderItem oi WHERE oi.order.user = :user AND oi.order.status = 'COMPLETED'")
    BigDecimal calculateTotalSpentByUser(@Param("user") User user);

    /**
     * Calculate total revenue by category name.
     * Used for category revenue analysis.
     * 
     * @param categoryName the category name
     * @return total revenue from the category
     */
    @Query("SELECT COALESCE(SUM(oi.price), 0) FROM OrderItem oi WHERE oi.categoryName = :categoryName AND oi.order.status = 'COMPLETED'")
    BigDecimal calculateTotalRevenueByCategoryName(@Param("categoryName") String categoryName);

    /**
     * Calculate average price by product.
     * Used for product pricing analysis.
     * 
     * @param product the product
     * @return average price for the product
     */
    @Query("SELECT COALESCE(AVG(oi.price), 0) FROM OrderItem oi WHERE oi.product = :product AND oi.order.status = 'COMPLETED'")
    ...
    /**
     * Find order items by completed orders.
     * Used for completed sales analysis.
     * 
     * @return list of order items from completed orders
     */
    @Query("SELECT oi FROM OrderItem oi WHERE oi.order.status = 'COMPLETED' ORDER BY oi.createdAt DESC")
     ...
     */
    @Query("SELECT DISTINCT oi.categoryName FROM OrderItem oi WHERE oi.categoryName IS NOT NULL ORDER BY oi.categoryName")
    List<String> findDistinctCategoryNames();

    /**
     * Find distinct servers.
     * Used for server filter options.
     * 
     * @return list of distinct server names
     */
    @Query("SELECT DISTINCT oi.server FROM OrderItem oi WHERE oi.server IS NOT NULL ORDER BY oi.server")
