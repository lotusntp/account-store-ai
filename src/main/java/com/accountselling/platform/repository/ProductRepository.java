...
/**
 * Repository interface for Product entity operations. Provides data access methods for product
 * management functionality including search, filtering, and inventory-related operations.
 *
 * <p>รีพอสิทอรี่สำหรับจัดการข้อมูลสินค้า รองรับการค้นหา การกรอง และการจัดการสต็อก
 */
@Repository
public interface ProductRepository extends JpaRepository<Product, UUID> {

  /**
   * Find product by name. Used for product lookup and validation.
   *
  ...
  /**
   * Check if product name exists. Used for product creation validation.
   *
  ...
  /**
   * Find products by active status. Used for displaying active products to customers.
   *
  ...
  /**
   * Find active products with pagination. Used for customer-facing product listings.
   *
  ...
  /**
   * Find products by category. Used for category-based product browsing.
   *
   * @param categoryId the category ID to filter by
   * @return list of products in the specified category
   */
  @Query(
      "SELECT p FROM Product p WHERE p.category.id = :categoryId ORDER BY p.sortOrder ASC, p.name"
          + " ASC")
  List<Product> findByCategoryId(@Param("categoryId") UUID categoryId);

  /**
   * Find active products by category with pagination. Used for customer-facing category browsing.
   *
   * @param categoryId the category ID to filter by
   * @param pageable pagination parameters
   ...
   */
  @Query(
      "SELECT p FROM Product p WHERE p.category.id = :categoryId AND p.active = true ORDER BY"
          + " p.sortOrder ASC, p.name ASC")
  Page<Product> findActiveByCategoryId(@Param("categoryId") UUID categoryId, Pageable pageable);

  /**
   * Find products by category hierarchy (including subcategories). Used for browsing products in a
   ...
   */
  @Query(
      "SELECT p FROM Product p WHERE p.category.id IN :categoryIds ORDER BY p.sortOrder ASC, p.name"
          + " ASC")
  List<Product> findByCategoryIdIn(@Param("categoryIds") List<UUID> categoryIds);

  /**
   * Find active products by category hierarchy with pagination. Used for customer-facing
   ...
   */
  @Query(
      "SELECT p FROM Product p WHERE p.category.id IN :categoryIds AND p.active = true ORDER BY"
          + " p.sortOrder ASC, p.name ASC")
  Page<Product> findActiveByCategoryIdIn(
      @Param("categoryIds") List<UUID> categoryIds, Pageable pageable);

  /**
   * Find products by name containing (case-insensitive). Used for product search functionality.
   *
  ...
  /**
   * Find active products by name containing with pagination. Used for customer-facing product
   * search.
   *
   * @param name the name pattern to search for
   * @param pageable pagination parameters
  ...
  /**
   * Find products by server. Used for server-specific product filtering.
   *
  ...
  /**
   * Find active products by server with pagination. Used for customer-facing server-based
   * filtering.
   *
   * @param server the server name to filter by
   * @param pageable pagination parameters
  ...
  /**
   * Find products by price range. Used for price-based filtering.
   *
   ...
   */
  @Query(
      "SELECT p FROM Product p WHERE p.price >= :minPrice AND p.price <= :maxPrice ORDER BY p.price"
          + " ASC")
  List<Product> findByPriceBetween(
      @Param("minPrice") BigDecimal minPrice, @Param("maxPrice") BigDecimal maxPrice);

  /**
   * Find active products by price range with pagination. Used for customer-facing price filtering.
   *
   * @param minPrice minimum price (inclusive)
   * @param maxPrice maximum price (inclusive)
   * @param pageable pagination parameters
   ...
   */
  @Query(
      "SELECT p FROM Product p WHERE p.price >= :minPrice AND p.price <= :maxPrice AND p.active ="
          + " true ORDER BY p.price ASC")
  Page<Product> findActiveByPriceBetween(
      @Param("minPrice") BigDecimal minPrice,
      @Param("maxPrice") BigDecimal maxPrice,
      Pageable pageable);

  /**
   * Search products by multiple criteria. Used for advanced product search with multiple filters.
   *
  ...
  /**
   * Find products with low stock. Used for inventory management and alerts.
   *
  ...
  /**
   * Find products that are out of stock. Used for inventory management.
   *
  ...
  /**
   * Find products with available stock. Used for customer-facing product listings.
   *
  ...
  /**
   * Find products with available stock by category with pagination. Used for customer-facing
   * category browsing with stock availability.
   *
   * @param categoryId the category ID to filter by
   * @param pageable pagination parameters
  ...
  @Query(
      ...
      ORDER BY p.sortOrder ASC, p.name ASC
      """)
  Page<Product> findProductsWithAvailableStockByCategoryId(
      @Param("categoryId") UUID categoryId, Pageable pageable);

  /**
   * Get distinct servers from all products. Used for server filter options.
   *
   * @return list of distinct server names
   */
  @Query(
      "SELECT DISTINCT p.server FROM Product p WHERE p.server IS NOT NULL AND p.active = true ORDER"
          + " BY p.server")
  List<String> findDistinctServers();

  /**
   * Find products ordered by name for admin interfaces. Used for consistent product listing in
   * admin panels.
   *
  ...
  /**
   * Find products ordered by sort order and name. Used for organized product display.
   *
  ...
  /**
   * Find products by category ordered by sort order and name. Used for organized category-based
   * product display.
   *
   * @param categoryId the category ID to filter by
  ...
  /**
   * Count products by category. Used for category statistics.
   *
  ...
  /**
   * Count active products by category. Used for customer-facing category statistics.
   *
