...
/**
 * Repository interface for Payment entity operations. Provides comprehensive data access methods
 * for payment management including transaction tracking, status monitoring, financial reporting,
 * and payment analytics.
 *
 * <p>รีพอสิทอรี่สำหรับจัดการข้อมูลการชำระเงิน รองรับการจัดการการชำระเงิน การติดตามธุรกรรม
 * การตรวจสอบสถานะ การรายงานทางการเงิน และการวิเคราะห์การชำระเงิน
 */
@Repository
public interface PaymentRepository extends JpaRepository<Payment, UUID> {
  ...
  /**
   * Find payment by order. Used for retrieving payment information for a specific order.
   *
  ...
  /**
   * Find payment by order ID. Used for order-specific payment lookup.
   *
  ...
  /**
   * Find payment by transaction ID. Used for payment gateway callback processing and transaction
   * lookup.
   *
  ...
  /**
   * Find payment by payment reference. Used for internal payment reference lookup.
   *
  ...
  /**
   * Check if transaction ID exists. Used for transaction ID validation and uniqueness checks.
   *
  ...
  /**
   * Check if payment reference exists. Used for payment reference validation and uniqueness checks.
   *
  ...
  /**
   * Find payments by status. Used for status-based payment management and monitoring.
   *
  ...
  /**
   * Find payments by status with pagination. Used for paginated status-based payment management.
   *
   * @param status the payment status to filter by
  ...
  /**
   * Find pending payments. Used for processing pending payments and timeout handling.
   *
  ...
  /**
   * Find processing payments. Used for monitoring payments currently being processed.
   *
  ...
  /**
   * Find completed payments. Used for financial reporting and analytics.
   *
  ...
  /**
   * Find failed payments. Used for failure analysis and customer support.
   *
  ...
  /**
   * Find cancelled payments. Used for cancellation tracking and analysis.
   *
  ...
  /**
   * Find refunded payments. Used for refund tracking and financial reconciliation.
   *
  ...
  /**
   * Find payments by user. Used for user payment history and analytics.
   *
  ...
  /**
   * Find payments by user with pagination. Used for paginated user payment history.
   *
   * @param user the user to search payments for
   * @param pageable pagination parameters
  ...
  /**
   * Find payments by user ID. Used for user-specific payment tracking.
   *
  ...
  /**
   * Find payments by user and status. Used for user-specific status-based payment filtering.
   *
   * @param user the user to search payments for
   * @param status the payment status to filter by
   ...
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.order.user = :user AND p.status = :status ORDER BY"
          + " p.createdAt DESC")
  List<Payment> findByUserAndStatus(
      @Param("user") User user, @Param("status") PaymentStatus status);

  // ==================== DATE-BASED QUERIES ====================

  /**
   * Find payments created within date range. Used for reporting and analytics within specific
   * periods.
   *
   ...
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.createdAt >= :startDate AND p.createdAt <= :endDate ORDER BY"
          + " p.createdAt DESC")
  List<Payment> findPaymentsBetweenDates(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  /**
   * Find completed payments within date range. Used for financial reporting within specific
   * periods.
   *
   * @param startDate start date of the range
   * @param endDate end date of the range
   ...
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.status = 'COMPLETED' AND p.paidAt >= :startDate AND p.paidAt"
          + " <= :endDate ORDER BY p.paidAt DESC")
  List<Payment> findCompletedPaymentsBetweenDates(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  /**
   * Find payments by user within date range. Used for user-specific payment history within periods.
   *
   * @param user the user to search payments for
   * @param startDate start date of the range
   * @param endDate end date of the range
   ...
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.order.user = :user AND p.createdAt >= :startDate AND"
          + " p.createdAt <= :endDate ORDER BY p.createdAt DESC")
  List<Payment> findPaymentsByUserBetweenDates(
  ...
  /**
   * Find expired payments. Used for cleanup operations and timeout handling.
   *
   * @return list of payments that have expired
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.expiresAt IS NOT NULL AND p.expiresAt <= CURRENT_TIMESTAMP"
          + " AND p.status IN ('PENDING', 'PROCESSING') ORDER BY p.expiresAt ASC")
  List<Payment> findExpiredPayments();

  /**
   * Find payments expiring soon. Used for proactive payment management and notifications.
   *
  ...
  @Query(
      "SELECT p FROM Payment p WHERE p.expiresAt IS NOT NULL AND p.expiresAt > CURRENT_TIMESTAMP"
          + " AND p.expiresAt <= :threshold AND p.status IN ('PENDING', 'PROCESSING') ORDER BY"
          + " p.expiresAt ASC")
  List<Payment> findPaymentsExpiringSoon(@Param("threshold") LocalDateTime threshold);

  /**
   * Find payments without expiration time. Used for data integrity checks and cleanup.
   *
   * @return list of payments without expiration time set
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.expiresAt IS NULL AND p.status IN ('PENDING', 'PROCESSING')")
  List<Payment> findPaymentsWithoutExpiration();

  // ==================== AMOUNT-BASED QUERIES ====================

  /**
   * Find payments by amount range. Used for amount-based filtering and analysis.
   *
   ...
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.amount >= :minAmount AND p.amount <= :maxAmount ORDER BY"
          + " p.amount DESC")
  List<Payment> findPaymentsByAmountRange(
      @Param("minAmount") BigDecimal minAmount, @Param("maxAmount") BigDecimal maxAmount);

  /**
   * Find payments above specified amount. Used for high-value payment tracking.
   *
  ...
  /**
   * Find payments by payment method. Used for payment method analysis and reporting.
   *
  ...
  /**
   * Find completed payments by payment method. Used for payment method performance analysis.
   *
   * @param paymentMethod the payment method to filter by
   * @return list of completed payments using the specified payment method
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.paymentMethod = :paymentMethod AND p.status = 'COMPLETED'"
          + " ORDER BY p.paidAt DESC")
  List<Payment> findCompletedPaymentsByMethod(@Param("paymentMethod") String paymentMethod);

  /**
   * Get distinct payment methods. Used for payment method filter options and analytics.
   *
   * @return list of distinct payment methods
   */
  @Query(
      "SELECT DISTINCT p.paymentMethod FROM Payment p WHERE p.paymentMethod IS NOT NULL ORDER BY"
          + " p.paymentMethod")
  ...
  /**
   * Count payments by status. Used for status-based statistics.
   *
  ...
  /**
   * Count payments by user. Used for user payment statistics.
   *
  ...
  /**
   * Count payments by user and status. Used for user-specific status statistics.
   *
   * @param user the user to count payments for
   * @param status the payment status to count
  ...
  /**
   * Count payments created within date range. Used for period-based statistics.
   *
   * @param startDate start date of the range
   * @param endDate end date of the range
   ...
   */
  @Query(
      "SELECT COUNT(p) FROM Payment p WHERE p.createdAt >= :startDate AND p.createdAt <= :endDate")
  long countPaymentsBetweenDates(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  /**
   * Count completed payments within date range. Used for financial statistics within periods.
   *
   * @param startDate start date of the range
   * @param endDate end date of the range
   ...
   */
  @Query(
      "SELECT COUNT(p) FROM Payment p WHERE p.status = 'COMPLETED' AND p.paidAt >= :startDate AND"
          + " p.paidAt <= :endDate")
  ...
  /**
   * Calculate total revenue from completed payments. Used for revenue reporting and analytics.
   *
   ...
   */
  @Query(
      "SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.status = 'COMPLETED' AND p.paidAt"
          + " >= :startDate AND p.paidAt <= :endDate")
  BigDecimal calculateRevenueBetweenDates(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  /**
   * Calculate average payment amount. Used for payment analytics.
   *
   ...
   */
  @Query(
      "SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.order.user = :user AND p.status ="
          + " 'COMPLETED'")
  BigDecimal calculateRevenueByUser(@Param("user") User user);

  /**
   * Calculate total refunded amount. Used for refund tracking and financial reconciliation.
   *
   * @return total amount refunded from all refunded payments
   */
  @Query(
      "SELECT COALESCE(SUM(p.refundAmount), 0) FROM Payment p WHERE p.status = 'REFUNDED' AND"
          + " p.refundAmount IS NOT NULL")
  BigDecimal calculateTotalRefundedAmount();

  /**
   * Calculate total refunded amount within date range. Used for period-based refund reporting.
   *
   * @param startDate start date of the range
   * @param endDate end date of the range
  ...
  @Query(
      "SELECT COALESCE(SUM(p.refundAmount), 0) FROM Payment p WHERE p.status = 'REFUNDED' AND"
          + " p.refundAmount IS NOT NULL AND p.refundedAt >= :startDate AND p.refundedAt <="
          + " :endDate")
  BigDecimal calculateRefundedAmountBetweenDates(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  // ==================== BULK OPERATIONS ====================

  /**
   * Mark expired payments as failed in bulk. Used for scheduled cleanup operations.
   *
  ...
  @Transactional
  @Query(
      "UPDATE Payment p SET p.status = 'FAILED', p.failureReason = :failureReason WHERE p.expiresAt"
          + " IS NOT NULL AND p.expiresAt <= CURRENT_TIMESTAMP AND p.status IN ('PENDING',"
          + " 'PROCESSING')")
  int markExpiredPaymentsAsFailed(@Param("failureReason") String failureReason);

  /**
   * Cancel payments for cancelled orders in bulk. Used for order cancellation processing.
   *
  ...
  @Transactional
  @Query(
      "UPDATE Payment p SET p.status = 'CANCELLED' WHERE p.order.id IN :orderIds AND p.status IN"
          + " ('PENDING', 'PROCESSING')")
  ...
  /**
   * Find recent payments for dashboard. Used for admin dashboard recent activity display.
   *
  ...
  /**
   * Find payments with gateway response issues. Used for payment gateway troubleshooting.
   *
   * @return list of payments with potential gateway issues
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.status = 'FAILED' AND p.gatewayResponse IS NOT NULL ORDER BY"
          + " p.createdAt DESC")
  List<Payment> findPaymentsWithGatewayIssues();

  /**
   * Find payments by username. Used for admin payment search functionality.
   *
   * @param username the username to search payments for
   * @return list of payments for the specified username
   */
  @Query(
      "SELECT p FROM Payment p WHERE p.order.user.username = :username ORDER BY p.createdAt DESC")
  List<Payment> findPaymentsByUsername(@Param("username") String username);

  /**
   * Find payments by username with pagination. Used for paginated admin payment search.
   *
   * @param username the username to search payments for
   * @param pageable pagination parameters
  ...
  /**
   * Search payments by multiple criteria. Used for advanced payment search with multiple filters.
   *
  ...
  /**
   * Get payment statistics for dashboard. Used for admin dashboard statistics display.
   *
  ...
  /**
   * Get daily payment statistics within date range. Used for daily payment reporting and charts.
   *
   * @param startDate start date of the range
   * @param endDate end date of the range
  ...
  /**
   * Get payment method statistics. Used for payment method performance analysis.
   *
  ...
  /**
   * Find payments by status in list. Used for multi-status filtering.
   *
  ...
  /**
   * Find payments by username. Used for user-based search.
   *
  ...
  /**
   * Find payments created between dates. Used for date range filtering.
   *
  ...
  /**
   * Find payments paid between dates. Used for payment date range filtering.
   *
   * @param startDate start date
   * @param endDate end date
  ...
  /**
   * Find payments by amount between range. Used for amount range filtering.
   *
  ...
  /**
   * Find payments by amount greater than. Used for minimum amount filtering.
   *
  ...
  /**
   * Count payments by payment method. Used for payment method statistics.
   *
  ...
  /**
   * Calculate total completed amount. Used for completed payment analysis.
   *
  ...
  /**
   * Calculate total amount by user. Used for user payment analysis.
   *
   * @param user the user
  ...
  /**
   * Find payments with QR codes. Used for QR code payment analysis.
   *
  ...
  /**
   * Find payments with failure reasons. Used for failure analysis.
   *
