...
/**
 * Standard error response record for all API endpoints. Provides consistent error information
 * structure throughout the application.
 *
 * <p>Error response มาตรฐานสำหรับทุก API endpoint ในระบบ
 */
@Schema(description = "Standard error response format")
public record ErrorResponse(
    @Schema(description = "HTTP status code", example = "400") int status,
    @Schema(description = "HTTP status text", example = "Bad Request") String error,
