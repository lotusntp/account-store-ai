...
/**
 * DTO for stock response information.
 * DTO สำหรับข้อมูลตอบกลับของสต็อก
 */
@Data
@Builder
@NoArgsConstructor
...
public class StockResponseDto {
    ...
    @Schema(description = "Account credentials (masked for security)", example = "username:***\\npassword:***")
    private String maskedCredentials;

    @Schema(description = "Additional information about the account", example = "Level 80 character with rare items")
    ...
    /**
     * Get status based on stock state.
     * รับสถานะตามสถานะของสต็อก
     */
    public String getStatus() {
        if (sold != null && sold) {
            return "SOLD";
        }
        if (reserved != null && reserved && reservedUntil != null && reservedUntil.isAfter(LocalDateTime.now())) {
            return "RESERVED";
        }
        return "AVAILABLE";
    }
     ...
     * ตรวจสอบว่าสต็อกถูกจองอยู่หรือไม่
     */
    public Boolean getReserved() {
        return reservedUntil != null && reservedUntil.isAfter(LocalDateTime.now()) && (sold == null || !sold);
    }
}