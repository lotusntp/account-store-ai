...
/**
 * DTO for creating multiple stock items in bulk.
 * DTO สำหรับการสร้างสต็อกหลายรายการพร้อมกัน
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
...
public class StockBulkCreateRequestDto {
    ...
    @Schema(description = "List of account credentials (each will be encrypted)", 
            example = "[\"username:player1\\npassword:pass1\", \"username:player2\\npassword:pass2\"]", 
            required = true)
    private List<@NotNull @Size(max = 2000, message = "Each credential entry cannot exceed 2000 characters") String> credentialsList;
}