...
/**
 * DTO for payment creation requests. Contains order ID, payment method, and expiration settings for
 * creating new payments.
 */
public record PaymentCreateRequestDto(
    @NotNull(message = "Order ID cannot be null") UUID orderId,
    @NotBlank(message = "Payment method cannot be blank") String paymentMethod,
    @Positive(message = "Expiration minutes must be positive") Integer expirationMinutes,
    String notes) {

  /**
   * Get effective expiration minutes. Returns the specified expiration minutes or a default value.
   *
  ...
  /**
   * Get normalized payment method. Returns the payment method in uppercase for consistency.
   *
