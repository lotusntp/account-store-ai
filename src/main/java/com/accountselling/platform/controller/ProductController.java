 ...
 * Provides endpoints for browsing products, product search, and filtering.
 * 
 * REST Controller สำหรับการจัดการสินค้า
 * ให้บริการ endpoints สำหรับการเรียกดูสินค้า การค้นหาสินค้า และการกรอง
 */
@RestController
@RequestMapping("/api/products")
@RequiredArgsConstructor
@Slf4j
@Tag(name = "Product Management", description = "Endpoints for product browsing, search, and filtering operations")
public class ProductController {
    ...
    @Operation(
        ...
        description = "Retrieve paginated list of products with optional filtering by active status and stock availability."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<Page<ProductResponseDto>> getAllProducts(
        ...
        log.info("Getting all products - page: {}, size: {}, activeOnly: {}, inStockOnly: {}", 
                page, size, activeOnly, inStockOnly);
        ...
        log.info("Retrieved {} products out of {} total", response.getNumberOfElements(), response.getTotalElements());
        return ResponseEntity.ok(response);
    }

    @Operation(
        summary = "Get product by ID",
        description = "Retrieve detailed information about a specific product including stock information."
    )
    @ApiResponses(value = {
    ...
    @Operation(
        ...
        description = "Retrieve paginated list of products within a specific category with optional subcategory inclusion."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Products retrieved successfully",
                    content = @Content(schema = @Schema(implementation = ProductResponseDto.class))),
    ...
    public ResponseEntity<Page<ProductResponseDto>> getProductsByCategory(
        ...
        log.info("Getting products by category: {} with includeSubcategories: {}, activeOnly: {}, inStockOnly: {}", 
                categoryId, includeSubcategories, activeOnly, inStockOnly);
        
        Pageable pageable = createPageable(page, size, sortBy, sortDirection);
        
        Page<Product> productPage;
        if (inStockOnly) {
            productPage = productService.findProductsWithAvailableStockByCategory(categoryId, includeSubcategories, pageable);
        } else {
            productPage = productService.findProductsByCategory(categoryId, includeSubcategories, activeOnly, pageable);
        }
    ...
    @Operation(
        ...
        description = "Advanced product search with multiple filtering criteria including name, category, price range, server, and stock status."
    )
    @ApiResponses(value = {
    ...
    @GetMapping("/search")
    public ResponseEntity<Page<ProductResponseDto>> searchProducts(@Valid ProductSearchRequestDto searchRequest) {
        
        log.info("Searching products with criteria: name={}, categoryId={}, server={}, priceRange=[{}-{}], inStock={}, activeOnly={}", 
                searchRequest.getName(), searchRequest.getCategoryId(), searchRequest.getServer(),
                searchRequest.getMinPrice(), searchRequest.getMaxPrice(), searchRequest.getInStock(), searchRequest.getActiveOnly());
        ...
        log.info("Search found {} products out of {} total", response.getNumberOfElements(), response.getTotalElements());
        return ResponseEntity.ok(response);
    }

    @Operation(
        summary = "Search products by name",
        description = "Simple product search by name with support for partial matching and Thai language."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Search completed successfully",
                    content = @Content(schema = @Schema(implementation = ProductResponseDto.class))),
    ...
    public ResponseEntity<Page<ProductResponseDto>> searchProductsByName(
        ...
        Page<Product> productPage = productService.searchProductsByName(name.trim(), activeOnly, pageable);
        Page<ProductResponseDto> response = productPage.map(this::convertToDto);
    ...
    public ResponseEntity<Page<ProductResponseDto>> getProductsByServer(
            @Parameter(description = "Server name", required = true)
    ...
    public ResponseEntity<Page<ProductResponseDto>> getProductsByPriceRange(
            ...
            @RequestParam @DecimalMin(value = "0.01", message = "Minimum price must be greater than 0") BigDecimal minPrice,
            @Parameter(description = "Maximum price (inclusive)", required = true)
            @RequestParam @DecimalMin(value = "0.01", message = "Maximum price must be greater than 0") BigDecimal maxPrice,
            @Parameter(description = "Page number (0-based)")
            @RequestParam(value = "page", defaultValue = "0") int page,
            @Parameter(description = "Page size (1-100)")
            @RequestParam(value = "size", defaultValue = "20") int size,
        ...
        log.info("Getting products by price range: [{} - {}] with activeOnly={}", minPrice, maxPrice, activeOnly);
        ...
        Page<Product> productPage = productService.findProductsByPriceRange(minPrice, maxPrice, activeOnly, pageable);
        Page<ProductResponseDto> response = productPage.map(this::convertToDto);
        
        log.info("Retrieved {} products in price range: [{} - {}]", response.getNumberOfElements(), minPrice, maxPrice);
        return ResponseEntity.ok(response);
    }
    ...
    /**
     * Convert Product entity to basic ProductResponseDto.
     * แปลง Product entity เป็น ProductResponseDto พื้นฐาน
     */
    ...
    /**
     * Convert Product entity to detailed ProductResponseDto.
     * แปลง Product entity เป็น ProductResponseDto แบบรายละเอียด
     */
