...
/**
 * REST Controller for category management operations. Provides endpoints for browsing categories,
 * category hierarchy, and category search.
 *
 * <p>REST Controller สำหรับการจัดการหมวดหมู่สินค้า ให้บริการ endpoints สำหรับการเรียกดูหมวดหมู่
 * ลำดับชั้นหมวดหมู่ และการค้นหาหมวดหมู่
 */
@RestController
...
@Tag(
    ...
    description = "Endpoints for category browsing and search operations")
public class CategoryController {
  ...
  @Operation(
      ...
      description =
          "Retrieve all categories with optional filtering by active status. Returns hierarchical"
              + " category structure.")
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  @Operation(
      ...
      description =
          "Retrieve detailed information about a specific category including its subcategories and"
              + " product count.")
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  @Operation(
      ...
      description =
          "Retrieve complete category hierarchy starting from root categories. Includes all"
              + " subcategories in tree structure.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<List<CategoryResponseDto>> getCategoryHierarchy(
    ...
    List<CategoryResponseDto> response =
        rootCategories.stream().map(this::convertToHierarchicalDto).collect(Collectors.toList());
  ...
  @Operation(
      ...
      description =
          "Retrieve only root categories (categories without parent). Useful for navigation menus.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<List<CategoryResponseDto>> getRootCategories(
    ...
    List<CategoryResponseDto> response =
        rootCategories.stream().map(this::convertToDto).collect(Collectors.toList());
  ...
  public ResponseEntity<List<CategoryResponseDto>> getSubcategories(
      @Parameter(description = "Parent category ID", required = true) @PathVariable UUID parentId,
  ...
  @Operation(
      ...
      description =
          "Search categories by name using case-insensitive pattern matching. Supports partial"
              + " matches.")
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  public ResponseEntity<List<CategoryResponseDto>> searchCategories(
      @Parameter(
              description = "Category name to search for (minimum 2 characters)",
              required = true)
          @RequestParam
  ...
  @Operation(
      ...
      description =
          "Get breadcrumb path from root to specified category. Returns list of categories in"
              + " hierarchy order.")
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  public ResponseEntity<List<CategoryResponseDto>> getCategoryPath(
    ...
    List<CategoryResponseDto> response =
        categoryPath.stream().map(this::convertToDto).collect(Collectors.toList());
  ...
  /**
   * Convert Category entity to hierarchical CategoryResponseDto with all descendants. แปลง Category
   * entity เป็น CategoryResponseDto แบบลำดับชั้นพร้อมลูกหลานทั้งหมด
   */
