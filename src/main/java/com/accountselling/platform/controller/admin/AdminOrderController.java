...
/**
 * Admin controller for order management operations. Provides order viewing, status management, and
 * statistics with admin privileges.
 *
 * <p>Admin controller สำหรับการจัดการคำสั่งซื้อ ให้บริการการดู การจัดการสถานะ และสถิติด้วยสิทธิ์
 * admin
 */
@RestController
...
@Tag(
    ...
    description = "Admin endpoints for order management and statistics")
...
public class AdminOrderController {
  ...
  public ResponseEntity<Page<OrderResponseDto>> searchOrders(
      @Parameter(description = "Order number pattern") @RequestParam(required = false)
          String orderNumber,
      ...
      @Parameter(description = "Minimum amount") @RequestParam(required = false)
          BigDecimal minAmount,
      @Parameter(description = "Maximum amount") @RequestParam(required = false)
    ...
    log.info(
        "Admin searching orders - orderNumber: {}, username: {}, status: {}, startDate: {},"
            + " endDate: {}",
        orderNumber,
        username,
        status,
    ...
    log.info(
        "Admin retrieved {} orders out of {} total",
  ...
  public ResponseEntity<OrderResponseDto> getOrderById(
    ...
    log.info(
        "Admin retrieved order: {} for user: {}",
  ...
  public ResponseEntity<Page<OrderResponseDto>> getOrdersByUser(
    ...
    log.info(
        "Admin retrieved {} orders for user: {}",
        response.getNumberOfElements(),
        user.getUsername());
    return ResponseEntity.ok(response);
  }
  ...
  public ResponseEntity<OrderResponseDto> markOrderAsFailed(
      ...
      @Parameter(description = "Failure reason")
          @RequestParam(required = false, defaultValue = "Order marked as failed by admin")
          String reason) {
  ...
  public ResponseEntity<OrderResponseDto> cancelOrder(
      ...
      @Parameter(description = "Cancellation reason")
          @RequestParam(required = false, defaultValue = "Order cancelled by admin")
          String reason) {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Daily order statistics retrieved successfully"),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  ...
  public ResponseEntity<List<DailyOrderStatistics>> getDailyOrderStatistics(
    ...
    List<DailyOrderStatistics> dailyStats =
  ...
  public ResponseEntity<List<UUID>> getTopCustomers(
    ...
    List<UUID> customerIds = topCustomers.stream().map(User::getId).collect(Collectors.toList());
  ...
  public ResponseEntity<List<OrderResponseDto>> getRecentOrders(
    ...
    List<OrderResponseDto> response =
        recentOrders.stream().map(this::convertToDto).collect(Collectors.toList());
  ...
  /** Convert Order entity to OrderResponseDto. แปลง Order entity เป็น OrderResponseDto */
