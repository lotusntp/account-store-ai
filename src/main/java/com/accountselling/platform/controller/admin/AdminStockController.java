...
/**
 * Admin controller for stock inventory management operations. Provides CRUD operations for stock
 * items with admin privileges.
 *
 * <p>Admin controller สำหรับการจัดการสต็อกสินค้า ให้บริการ CRUD operations สำหรับสต็อกด้วยสิทธิ์
 * admin
 */
@RestController
...
@Tag(
    ...
    description = "Admin endpoints for stock inventory CRUD operations")
...
public class AdminStockController {
  ...
  @Operation(
      ...
      description =
          "Retrieve all stock items for a specific product with pagination. Admin only endpoint.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<Page<StockResponseDto>> getStockByProduct(
    ...
    log.info(
        "Admin retrieved {} stock items for product: {}",
  ...
  @Operation(
      ...
      description =
          "Retrieve detailed information about a specific stock item. Admin only endpoint.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<StockResponseDto> getStockById(
      @Parameter(description = "Stock ID", required = true) @PathVariable UUID id) {
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "400",
            description = "Invalid stock data or credentials already exist"),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
        @ApiResponse(responseCode = "404", description = "Product not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  ...
  public ResponseEntity<StockResponseDto> createStock(
      @Parameter(description = "Stock creation data", required = true) @Valid @RequestBody
          StockCreateRequestDto request) {

    log.info("Admin creating stock for product: {}", request.getProductId());

    Stock createdStock =
        stockService.createStock(
            request.getProductId(), request.getCredentials(), request.getAdditionalInfo());

    StockResponseDto response = convertToDto(createdStock);

    log.info(
        "Admin created stock item for product: {} with id: {}",
        createdStock.getProduct().getName(),
        createdStock.getId());
  ...
  @Operation(
      ...
      description =
          "Create multiple stock items for a product in a single operation. Admin only endpoint.")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<List<StockResponseDto>> createBulkStock(
      @Parameter(description = "Bulk stock creation data", required = true) @Valid @RequestBody
          StockBulkCreateRequestDto request) {

    log.info(
        "Admin creating {} stock items for product: {}",
        request.getCredentialsList().size(),
        request.getProductId());

    List<Stock> createdStocks =
        stockService.createBulkStock(request.getProductId(), request.getCredentialsList());

    List<StockResponseDto> response =
        createdStocks.stream().map(this::convertToDto).collect(Collectors.toList());

    log.info(
        "Admin created {} stock items for product: {}", response.size(), request.getProductId());
    return ResponseEntity.status(HttpStatus.CREATED).body(response);
  }
  ...
  public ResponseEntity<StockResponseDto> updateStock(
      @Parameter(description = "Stock ID", required = true) @PathVariable UUID id,
      @Parameter(description = "Stock update data", required = true) @Valid @RequestBody
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Stock item marked as sold successfully",
            content = @Content(schema = @Schema(implementation = StockResponseDto.class))),
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "400",
            description = "Stock item cannot be deleted (sold or reserved)"),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
        @ApiResponse(responseCode = "404", description = "Stock item not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Stock statistics retrieved successfully",
  ...
  public ResponseEntity<List<UUID>> getProductsWithLowStock(
    ...
    List<UUID> productIds =
        lowStockProducts.stream().map(Product::getId).collect(Collectors.toList());
  ...
  public ResponseEntity<List<StockResponseDto>> getExpiredReservations() {
    ...
    List<StockResponseDto> response =
        expiredReservations.stream().map(this::convertToDto).collect(Collectors.toList());
  ...
  @Operation(
      ...
      description =
          "Clean up all expired reservations and make stock available again. Admin only endpoint.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @Operation(
      ...
      description =
          "Get list of sold stock items with optional date filtering. Admin only endpoint.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Sold stock retrieved successfully",
            content = @Content(schema = @Schema(implementation = StockResponseDto.class))),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  ...
  public ResponseEntity<List<StockResponseDto>> getSoldStock(
      @Parameter(
              description =
                  "Product ID (optional - gets sold stock for all products if not specified)")
          @RequestParam(required = false)
          UUID productId,
    ...
    log.info(
        "Admin getting sold stock - productId: {}, startDate: {}, endDate: {}",
        productId,
        startDate,
        endDate);
    ...
    List<StockResponseDto> response =
        soldStock.stream().map(this::convertToDto).collect(Collectors.toList());
  ...
  /** Convert Stock entity to StockResponseDto. แปลง Stock entity เป็น StockResponseDto */
  private StockResponseDto convertToDto(Stock stock) {
    return StockResponseDto.builder()
        .id(stock.getId())
        .product(
            StockResponseDto.ProductInfo.builder()
  ...
  /** Mask sensitive credentials for display. ปิดบังข้อมูลสำคัญสำหรับการแสดงผล */
