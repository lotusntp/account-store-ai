 ...
 * Provides CRUD operations for categories with admin privileges.
 * 
 * Admin controller สำหรับการจัดการหมวดหมู่
 * ให้บริการ CRUD operations สำหรับหมวดหมู่ด้วยสิทธิ์ admin
 */
@RestController
@RequestMapping("/api/admin/categories")
@RequiredArgsConstructor
@Slf4j
@Tag(name = "Admin Category Management", description = "Admin endpoints for category CRUD operations")
...
public class AdminCategoryController {
    ...
    public ResponseEntity<CategoryResponseDto> createCategory(
        ...
        log.info("Admin created category: {} with id: {}", createdCategory.getName(), createdCategory.getId());
    ...
    @Operation(
        ...
        description = "Move a category to a new parent category or make it a root category. Admin only endpoint."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Category moved successfully",
                    content = @Content(schema = @Schema(implementation = CategoryResponseDto.class))),
        @ApiResponse(responseCode = "400", description = "Invalid move operation (would create circular dependency)"),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
    ...
    public ResponseEntity<CategoryResponseDto> moveCategory(
            @Parameter(description = "Category ID to move", required = true)
            @PathVariable UUID id,
    ...
    @Operation(
        ...
        description = "Delete a category if it has no products or subcategories. Admin only endpoint."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "204", description = "Category deleted successfully"),
        @ApiResponse(responseCode = "400", description = "Category cannot be deleted (has products or subcategories)"),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
        @ApiResponse(responseCode = "404", description = "Category not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    ...
    @Operation(
        ...
        description = "Delete a category and move its products to another category. Admin only endpoint."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "204", description = "Category deleted and products moved successfully"),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
    ...
    public ResponseEntity<Void> forceDeleteCategory(
        ...
        log.info("Admin force deleted category: {} and moved products to category: {}", 
                category.getName(), targetCategoryId);
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<Void> reorderCategories(
            ...
            @RequestParam(required = false) UUID parentId,
            @Parameter(description = "Ordered list of category IDs", required = true)
