 ...
 * Provides CRUD operations for users with admin privileges.
 * 
 * Admin controller สำหรับการจัดการผู้ใช้
 * ให้บริการ CRUD operations สำหรับผู้ใช้ด้วยสิทธิ์ admin
 */
@RestController
...
public class AdminUserController {
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "User updated successfully",
                    content = @Content(schema = @Schema(implementation = AdminUserResponseDto.class))),
        @ApiResponse(responseCode = "400", description = "Invalid user data or no fields to update"),
        @ApiResponse(responseCode = "403", description = "Access denied - admin role required"),
        @ApiResponse(responseCode = "404", description = "User not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    ...
    public ResponseEntity<AdminUserResponseDto> addRoleToUser(
            @Parameter(description = "User ID", required = true)
            @PathVariable UUID id,
            @Parameter(description = "Role name to add", required = true)
    ...
    public ResponseEntity<UserStatistics> getUserStatistics() {
        ...
        log.info("Admin retrieved user statistics: total={}, enabled={}, disabled={}", 
                totalUsers, enabledUsers, disabledUsers);
        return ResponseEntity.ok(stats);
    }

    /**
     * Convert User entity to AdminUserResponseDto.
     * แปลง User entity เป็น AdminUserResponseDto
     */
