...
/**
 * Implementation of OrderService for order management operations. Provides comprehensive order
 * processing functionality including creation, status management, payment integration, and business
 * logic operations with proper transaction management and concurrent access protection.
 */
@Slf4j
@Service
...
public class OrderServiceImpl implements OrderService {
  ...
  public Order createOrder(User user, Map<UUID, Integer> productQuantities) {
    log.info(
        "Creating order for user: {} with {} products",
        user.getUsername(),
    ...
    try {
      ...
      log.info(
          "Successfully created order: {} for user: {} with total: {}",
      ...
      log.error(
          "Failed to reserve stock for order creation. Rolling back order: {}",
          savedOrder.getOrderNumber(),
          e);
  ...
  public Order createOrder(User user, UUID productId, int quantity) {
    log.info(
        "Creating single product order for user: {} with product: {} quantity: {}",
        user.getUsername(),
        productId,
        quantity);
  ...
  public Order createOrderByUsername(String username, Map<UUID, Integer> productQuantities) {
    log.info(
        "Creating order for username: {} with {} products", username, productQuantities.size());
  ...
  public Order markOrderAsProcessing(UUID orderId) {
    ...
    try {
      ...
      log.error(
          "Cannot transition order {} to processing status: {}",
  ...
  public Order markOrderAsCompleted(UUID orderId) {
    ...
    try {
      ...
      log.error(
          "Cannot transition order {} to completed status: {}",
          order.getOrderNumber(),
          e.getMessage());
  ...
  public Order markOrderAsFailed(UUID orderId, String reason) {
    ...
    try {
      ...
      log.error(
          "Cannot transition order {} to failed status: {}",
          order.getOrderNumber(),
          e.getMessage());
  ...
  public Order cancelOrder(UUID orderId, String reason) {
    ...
    if (!order.canBeCancelled()) {
      log.error(
          "Order cannot be cancelled: {} with status: {}",
          order.getOrderNumber(),
          order.getStatus());
    ...
    try {
      ...
      log.error(
          "Cannot transition order {} to cancelled status: {}",
          order.getOrderNumber(),
          e.getMessage());
  ...
  public Order cancelOrderByUser(UUID orderId, User user, String reason) {
    log.info("User {} cancelling order: {} with reason: {}", user.getUsername(), orderId, reason);
  ...
  public void validateOrder(User user, Map<UUID, Integer> productQuantities) {
    log.debug(
        "Validating order for user: {} with {} products",
        user.getUsername(),
        productQuantities.size());
    ...
    for (Map.Entry<UUID, Integer> entry : productQuantities.entrySet()) {
      ...
      if (availableQuantity < quantity) {
        ...
        throw new InsufficientStockException(
            String.format(
                "Insufficient stock for product. Required: %d, Available: %d",
  ...
  public void reserveStockForOrder(Order order, Map<UUID, Integer> productQuantities) {
    ...
    log.info(
        "Successfully reserved {} stock items for order: {}",
  ...
  public void markStockItemsAsSold(UUID orderId) {
    ...
    log.info(
        "Successfully marked {} stock items as sold for order: {}",
        order.getOrderItems().size(),
        order.getOrderNumber());
  }
  ...
  public Page<Order> searchOrders(
      ...
      Pageable pageable) {
    log.debug(
        "Searching orders with criteria - orderNumber: {}, username: {}, status: {}",
        orderNumber,
        username,
        status);
  ...
  public OrderStatistics getOrderStatistics() {
    ...
    return new OrderStatistics(
        total, pending, processing, completed, failed, cancelled, totalRevenue, averageOrderValue);
  }
        ...
        .map(
            stat ->
                new DailyOrderStatistics(
                    (LocalDateTime) stat[0], ((Number) stat[1]).longValue(), (BigDecimal) stat[2]))
        .toList();
  }
  ...
  public void processOrderCompletion(UUID orderId, String paymentTransactionId) {
    log.info(
        "Processing order completion for order: {} with transaction: {}",
        orderId,
    ...
    log.info(
        "Successfully processed order completion: {} with transaction: {}",
  ...
  public void processOrderFailure(UUID orderId, String failureReason) {
    ...
    log.info(
        "Successfully processed order failure: {} with reason: {}",
  ...
  public Map<String, String> getOrderDownloadInfo(UUID orderId, User user) {
    ...
    log.info(
        "Successfully retrieved download info for {} products in order: {}",
        downloadInfo.size(),
        order.getOrderNumber());
