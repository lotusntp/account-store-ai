...
/**
 * Implementation of PaymentService for payment management operations.
...
public class PaymentServiceImpl implements PaymentService {
    ...
    public Payment createPayment(Order order, String paymentMethod, int expirationMinutes) {
        log.info("Creating payment for order: {} with method: {} expiration: {} minutes", 
                order.getOrderNumber(), paymentMethod, expirationMinutes);
        ...
        log.info("Successfully created payment: {} for order: {}", 
                savedPayment.getPaymentReference(), order.getOrderNumber());
    ...
    public String generateQrCode(Payment payment) {
        ...
        try {
            ...
            String qrCodeContent = String.format("%s?ref=%s&amount=%.2f&expires=%s", 
                                                qrCodeBaseUrl,
    ...
    public Payment regenerateQrCode(UUID paymentId) {
        ...
        if (!payment.isPending() && !payment.isProcessing()) {
            ...
            throw new InvalidPaymentStatusException("Cannot regenerate QR code for payment in status: " + payment.getStatus());
        }
    ...
    public String getQrCodeContent(UUID paymentId) {
        ...
        if (payment.getQrCodeUrl() == null || payment.getQrCodeUrl().isEmpty()) {
            ...
            throw new ResourceNotFoundException("No QR code available for payment: " + payment.getPaymentReference());
        }
    ...
    public Payment findByOrder(Order order) {
        ...
        return paymentRepository.findByOrder(order)
            .orElseThrow(() -> {
                ...
                return new ResourceNotFoundException("Payment not found for order: " + order.getOrderNumber());
            });
    }
    ...
    public Payment markPaymentAsProcessing(UUID paymentId) {
        ...
        try {
            ...
            log.error("Cannot transition payment {} to processing status: {}", 
                     payment.getPaymentReference(), e.getMessage());
            throw new InvalidPaymentStatusException("Cannot mark payment as processing: " + e.getMessage());
        }
    }

    @Override
    @Transactional
    public Payment markPaymentAsCompleted(UUID paymentId, String transactionId, String gatewayResponse) {
        ...
        try {
            ...
            log.error("Cannot transition payment {} to completed status: {}", 
                     payment.getPaymentReference(), e.getMessage());
            throw new InvalidPaymentStatusException("Cannot mark payment as completed: " + e.getMessage());
        }
    }
    ...
    public Payment markPaymentAsFailed(UUID paymentId, String failureReason, String gatewayResponse) {
        ...
        try {
            ...
            log.error("Cannot transition payment {} to failed status: {}", 
                     payment.getPaymentReference(), e.getMessage());
    ...
    public Payment cancelPayment(UUID paymentId, String reason) {
        ...
        if (!payment.canBeCancelled()) {
            log.error("Payment cannot be cancelled: {} with status: {}", 
                     payment.getPaymentReference(), payment.getStatus());
            throw new InvalidPaymentStatusException("Payment cannot be cancelled in current status: " + payment.getStatus());
        }
        ...
        try {
            ...
            log.error("Cannot transition payment {} to cancelled status: {}", 
                     payment.getPaymentReference(), e.getMessage());
    ...
    public Payment processRefund(UUID paymentId, BigDecimal refundAmount, String reason) {
        ...
        if (!payment.canBeRefunded()) {
            log.error("Payment cannot be refunded: {} with status: {}", 
                     payment.getPaymentReference(), payment.getStatus());
            throw new InvalidPaymentStatusException("Payment cannot be refunded in current status: " + payment.getStatus());
        }
        
        if (!isRefundAmountValid(paymentId, refundAmount)) {
            log.error("Invalid refund amount: {} for payment: {}", 
        ...
        try {
            ...
            log.info("Successfully processed refund: {} for payment: {}", 
                    refundAmount, payment.getPaymentReference());
            return updatedPayment;
            
        } catch (IllegalStateException e) {
            log.error("Cannot transition payment {} to refunded status: {}", 
                     payment.getPaymentReference(), e.getMessage());
        ...
        try {
            switch (status.toUpperCase()) {
                ...
                case "FAILED", "FAILURE", "ERROR" -> {
                    return markPaymentAsFailed(payment.getId(), "Payment failed via webhook", gatewayResponse);
                }
    ...
    public void validatePayment(Order order, String paymentMethod) {
        log.debug("Validating payment for order: {} with method: {}", order.getOrderNumber(), paymentMethod);
        ...
        if (order.isCompleted() || order.isFailed() || order.isCancelled()) {
            ...
            throw new InvalidPaymentException("Cannot create payment for order in status: " + order.getStatus());
        }
    ...
    public Payment extendPaymentExpiration(UUID paymentId, int additionalMinutes) {
        ...
        if (!payment.isPending() && !payment.isProcessing()) {
            ...
            throw new InvalidPaymentStatusException("Cannot extend expiration for payment in status: " + payment.getStatus());
        }
        ...
        log.info("Successfully extended payment expiration: {} by {} minutes", 
                payment.getPaymentReference(), additionalMinutes);
    ...
    @Override
    public Page<Payment> searchPayments(String transactionId, String paymentReference, String username,
                                      PaymentStatus status, String paymentMethod,
                                      LocalDateTime startDate, LocalDateTime endDate,
                                      BigDecimal minAmount, BigDecimal maxAmount,
                                      Pageable pageable) {
        log.debug("Searching payments with criteria - transactionId: {}, reference: {}, username: {}", 
                 transactionId, paymentReference, username);
        
        return paymentRepository.searchPayments(transactionId, paymentReference, username,
    ...
    public PaymentStatistics getPaymentStatistics() {
        ...
        return new PaymentStatistics(total, pending, processing, completed, failed, cancelled, refunded,
                                   totalRevenue, totalRefunded, averagePaymentAmount, successRate);
    }
    ...
    public PaymentStatistics getPaymentStatistics(LocalDateTime startDate, LocalDateTime endDate) {
        ...
        BigDecimal totalRefunded = paymentRepository.calculateRefundedAmountBetweenDates(startDate, endDate);
        BigDecimal averagePaymentAmount = total > 0 ? totalRevenue.divide(BigDecimal.valueOf(total), 2, java.math.RoundingMode.HALF_UP) : BigDecimal.ZERO;
    ...
    @Override
    public List<DailyPaymentStatistics> getDailyPaymentStatistics(LocalDateTime startDate, LocalDateTime endDate) {
        ...
        return dailyStats.stream()
            .map(stat -> new DailyPaymentStatistics(
