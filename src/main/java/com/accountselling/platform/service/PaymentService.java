...
/**
 * Service interface for Payment management operations. Provides comprehensive payment processing
 * functionality including creation, QR code generation, status management, payment gateway
 * integration, and financial reporting.
 *
 * <p>This service handles: - Payment creation and QR code generation - Payment status lifecycle
 * management - Payment gateway webhook processing - Payment validation and verification - Financial
 * reporting and analytics - Payment method management
 */
public interface PaymentService {
  ...
  /**
   * Create a new payment for an order with QR code generation. This method creates a payment record
   * and generates a QR code for payment processing.
   *
   ...
   * @throws com.accountselling.platform.exception.PaymentAlreadyExistsException if payment already
   *     exists for order
   */
  Payment createPayment(Order order, String paymentMethod, int expirationMinutes);

  /**
   * Create a new payment for an order with default expiration time. Uses system default expiration
   * time for payment.
   *
   * @param order the order to create payment for
   * @param paymentMethod the payment method to use
  ...
  /**
   * Generate QR code for payment. Creates a QR code URL/content for payment processing.
   *
   * @param payment the payment to generate QR code for
   * @return the QR code URL or content
   * @throws com.accountselling.platform.exception.QrCodeGenerationException if QR code generation
   *     fails
   */
  String generateQrCode(Payment payment);

  /**
   * Regenerate QR code for existing payment. Useful when payment QR code expires or needs to be
   * refreshed.
   *
   ...
   * @throws com.accountselling.platform.exception.QrCodeGenerationException if QR code generation
   *     fails
   * @throws com.accountselling.platform.exception.InvalidPaymentStatusException if payment cannot
   *     be regenerated
   */
  Payment regenerateQrCode(UUID paymentId);

  /**
   * Get QR code content for payment. Returns the QR code content for display or processing.
   *
  ...
  /**
   * Find payment by transaction ID. Used for payment gateway callback processing.
   *
  ...
  /**
   * Mark payment as processing. This typically happens when payment gateway starts processing.
   *
   * @param paymentId the payment ID to update
   * @return the updated payment
   * @throws com.accountselling.platform.exception.ResourceNotFoundException if payment not found
   * @throws com.accountselling.platform.exception.InvalidPaymentStatusException if status
   *     transition is invalid
   */
  Payment markPaymentAsProcessing(UUID paymentId);

  /**
   * Mark payment as completed. This happens when payment is successfully processed by gateway.
   *
   * @param paymentId the payment ID to update
  ...
  /**
   * Mark payment as failed. This happens when payment processing fails.
   *
   * @param paymentId the payment ID to update
  ...
  /**
   * Cancel payment. This cancels a pending or processing payment.
   *
  ...
  /**
   * Process refund for completed payment. This creates a refund for a completed payment.
   *
   ...
   * @throws com.accountselling.platform.exception.InvalidPaymentStatusException if payment cannot
   *     be refunded
   * @throws com.accountselling.platform.exception.InvalidRefundException if refund amount is
   *     invalid
   */
  ...
  /**
   * Process payment gateway webhook. Handles incoming webhook notifications from payment gateway.
   *
   ...
   * @return the updated payment
   * @throws com.accountselling.platform.exception.ResourceNotFoundException if payment not found
   * @throws com.accountselling.platform.exception.WebhookProcessingException if webhook processing
   *     fails
   */
  Payment processWebhook(String transactionId, String status, String gatewayResponse);

  /**
   * Verify payment with gateway. Verifies payment status directly with payment gateway.
   *
   * @param paymentId the payment ID to verify
   * @return the updated payment with verified status
   * @throws com.accountselling.platform.exception.ResourceNotFoundException if payment not found
   * @throws com.accountselling.platform.exception.PaymentVerificationException if verification
   *     fails
   */
  Payment verifyPaymentWithGateway(UUID paymentId);

  /**
   * Check payment status with gateway. Queries payment gateway for current payment status.
   *
  ...
  /**
   * Validate payment before creation. Checks order eligibility and payment constraints.
   *
   * @param order the order to create payment for
  ...
  /**
   * Process expired payments. Marks expired payments as failed and releases associated resources.
  ...
  /**
   * Get payments expiring soon. Returns payments that will expire within specified minutes.
   *
  ...
  /**
   * Extend payment expiration. Extends the expiration time for a payment.
   *
   ...
   */
  List<DailyPaymentStatistics> getDailyPaymentStatistics(
