...
class PaymentServiceImplTest {
  ...
  void setUp() {
    ...
    ReflectionTestUtils.setField(
        paymentService, "qrCodeBaseUrl", "https://payment-gateway.example.com/qr");
  }
  ...
  void findById_WithInvalidId_ShouldThrowException() {
    ...
    // Act & Assert
    assertThrows(ResourceNotFoundException.class, () -> paymentService.findById(invalidId));
  }
  ...
  void markPaymentAsCompleted_WithValidPayment_ShouldUpdateStatusAndProcessOrder() {
    ...
    // Act
    Payment result =
        paymentService.markPaymentAsCompleted(testPayment.getId(), "TXN-123", "Gateway response");
  ...
  void markPaymentAsFailed_WithValidPayment_ShouldUpdateStatusAndProcessOrder() {
    ...
    Payment result =
        paymentService.markPaymentAsFailed(
            testPayment.getId(), "Payment failed", "Gateway response");
  ...
  void processRefund_WithValidPayment_ShouldProcessRefund() {
    ...
    // Act
    Payment result =
        paymentService.processRefund(testPayment.getId(), refundAmount, "Partial refund");
    ...
    assertThrows(
        ...
        () ->
            paymentService.processRefund(
                testPayment.getId(), invalidRefundAmount, "Invalid refund"));
  }
  ...
  void verifyPaymentWithGateway_WithValidPayment_ShouldVerifyAndUpdate() {
    ...
    when(paymentRepository.findById(testPayment.getId())).thenReturn(Optional.of(testPayment));
    when(paymentRepository.findByTransactionId(testPayment.getTransactionId()))
  ...
  void validatePayment_WithNullPaymentMethod_ShouldThrowException() {
    // Act & Assert
    assertThrows(
        InvalidPaymentException.class, () -> paymentService.validatePayment(testOrder, null));
  }
  ...
  void validatePayment_WithCompletedOrder_ShouldThrowException() {
    ...
    // Act & Assert
    assertThrows(
        InvalidPaymentException.class, () -> paymentService.validatePayment(testOrder, "QR_CODE"));
  }
  ...
  void getPaymentsExpiringSoon_WithThreshold_ShouldReturnPayments() {
    ...
    when(paymentRepository.findPaymentsExpiringSoon(any(LocalDateTime.class)))
        .thenReturn(expiringSoonPayments);
  ...
  void getPaymentStatistics_ShouldReturnStatistics() {
    ...
    Object[] stats = {
      10L, 3L, 5L, 2L, 1L, 1L
    }; // total, pending, completed, failed, cancelled, refunded
  ...
  void calculateRevenue_WithDateRange_ShouldReturnRevenue() {
    ...
    when(paymentRepository.calculateRevenueBetweenDates(startDate, endDate))
