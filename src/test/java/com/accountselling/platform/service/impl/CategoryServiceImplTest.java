...
/**
 * Unit tests for CategoryServiceImpl. Tests category management functionality including hierarchy
 * operations, search, and validation.
 *
 * <p>CategoryServiceImpl testing Tests category management functions including hierarchy
 * management, search, and validation
 */
...
class CategoryServiceImplTest {
  ...
  void setUp() {
    ...
    subCategory =
        new Category("MMORPG", "Massively Multiplayer Online Role-Playing Games", parentCategory);
  ...
  void updateCategory_WithValidData_ShouldUpdateCategory() {
    ...
    Category result =
        categoryService.updateCategory(categoryId, newName, newDescription, newSortOrder);
  ...
  void updateCategory_WithNonExistentCategory_ShouldThrowException() {
    ...
    assertThatThrownBy(
            () -> categoryService.updateCategory(categoryId, "New Name", "description", 1))
        .isInstanceOf(ResourceNotFoundException.class)
  ...
  void reorderCategories_WithValidCategories_ShouldUpdateSortOrder() {
    ...
    when(categoryRepository.saveAll(anyList()))
        .thenReturn(Arrays.asList(subCategory, parentCategory));
