...
/**
 * Comprehensive unit tests for OrderRepository. Tests all order management operations including
 * order tracking, status management, user orders, and reporting functionality.
 *
 * <p>OrderRepository testing covering order management status tracking, user orders, and reporting
 */
@DataJpaTest
@ActiveProfiles("test")
...
class OrderRepositoryTest {
  ...
  void shouldFindOrdersByUserId() {
    ...
    assertThat(result).hasSize(3);
    assertThat(result)
        .allSatisfy(order -> assertThat(order.getUser().getId()).isEqualTo(testUser1.getId()));
  }
  ...
  void shouldFindOrdersByUserWithinDateRange() {
    ...
    // When
    List<Order> result =
        orderRepository.findByUserAndCreatedAtBetween(testUser1, startDate, endDate);
  ...
  void shouldFindOrdersByStatusWithinDateRange() {
    ...
    // When
    List<Order> result =
        orderRepository.findByStatusAndCreatedAtBetween(OrderStatus.COMPLETED, startDate, endDate);
  ...
  void shouldCountOrdersByUserAndStatus() {
    // When
    long user1CompletedCount =
        orderRepository.countByUserAndStatus(testUser1, OrderStatus.COMPLETED);
  ...
  void shouldFindOrdersByUsername() {
    ...
    assertThat(result).hasSize(2);
    assertThat(result)
        .allSatisfy(order -> assertThat(order.getUser().getUsername()).isEqualTo("testuser2"));
  }
  ...
  void shouldHandleNullAndEmptySearchParameters() {
    ...
    Page<Order> result =
        orderRepository.searchOrders(null, null, null, null, null, null, null, pageable);
