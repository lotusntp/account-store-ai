...
/**
 * Unit tests for ProductRepository. Tests repository methods for product management functionality
 * including search, filtering, and inventory-related operations.
 */
@DataJpaTest
@ActiveProfiles("test")
class ProductRepositoryTest {
  ...
  void findByNameContainingIgnoreCaseAndActive_WithPagination_ShouldReturnMatchingActiveProducts() {
    ...
    // When
    Page<Product> result =
        productRepository.findByNameContainingIgnoreCaseAndActive("game", true, pageable);

    // Then
    assertThat(result.getContent()).hasSize(0); // "WoW Account" doesn't contain "game"

    // Test with "wow"
    Page<Product> wowResult =
        productRepository.findByNameContainingIgnoreCaseAndActive("wow", true, pageable);
  ...
  void findByPriceBetween_ShouldReturnProductsInPriceRange() {
    ...
    List<Product> midRangeProducts =
        productRepository.findByPriceBetween(new BigDecimal("25.00"), new BigDecimal("100.00"));
  ...
  void findActiveByPriceBetween_WithPagination_ShouldReturnActiveProductsInPriceRange() {
    ...
    // When
    Page<Product> result =
        productRepository.findActiveByPriceBetween(
  ...
  @Test
  void
      findProductsWithAvailableStockByCategoryId_WithPagination_ShouldReturnProductsWithStockInCategory() {
    // Given
    entityManager.persistAndFlush(gameCategory);
    entityManager.persistAndFlush(activeProduct);
    ...
    // When
    Page<Product> result =
        productRepository.findProductsWithAvailableStockByCategoryId(
  ...
  void findDistinctServers_ShouldReturnUniqueServerNames() {
    ...
    Product anotherStormrageProduct =
        new Product("Another WoW", new BigDecimal("60.00"), gameCategory);
