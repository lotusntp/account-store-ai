...
/**
 * Comprehensive unit tests for PaymentRepository.
...
class PaymentRepositoryTest {
    ...
    void shouldFindPaymentsByUserAndStatus() {
        // When
        List<Payment> result = paymentRepository.findByUserAndStatus(testUser1, PaymentStatus.COMPLETED);
    ...
    void shouldFindPaymentsCreatedWithinDateRange() {
        ...
        assertThat(result).allSatisfy(payment -> 
            assertThat(payment.getCreatedAt()).isBetween(startDate, endDate)
        );
    }
    ...
    void shouldFindPaymentsByAmountRange() {
        ...
        assertThat(result).allSatisfy(payment -> 
            assertThat(payment.getAmount()).isBetween(minAmount, maxAmount)
        );
    }
    ...
    void shouldFindPaymentsWithAmountGreaterThan() {
        ...
        assertThat(result).allSatisfy(payment -> 
            assertThat(payment.getAmount()).isGreaterThan(threshold)
        );
    }
    ...
    void shouldCalculateTotalAmountByStatus() {
        // When
        BigDecimal completedTotal = paymentRepository.calculateTotalAmountByStatus(PaymentStatus.COMPLETED);
