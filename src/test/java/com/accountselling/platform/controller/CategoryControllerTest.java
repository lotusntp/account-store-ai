 ...
 * 
 * Unit tests สำหรับ CategoryController
...
class CategoryControllerTest {
    ...
    void shouldGetCategoryByIdSuccessfully() throws Exception {
        ...
        when(categoryService.findActiveSubcategories(rootCategoryId)).thenReturn(Arrays.asList(subCategory));
        when(categoryService.countProductsInCategory(any(UUID.class), anyBoolean())).thenReturn(5L);

        // When & Then
        mockMvc.perform(get("/api/categories/{id}", rootCategoryId)
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    void shouldGetAllRootCategoriesWhenActiveOnlyIsFalse() throws Exception {
        ...
        when(categoryService.findRootCategories()).thenReturn(Arrays.asList(rootCategory, inactiveRoot));
        when(categoryService.countProductsInCategory(any(UUID.class), anyBoolean())).thenReturn(0L);
    ...
    void shouldGetSubcategoriesSuccessfully() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/categories/{parentId}/subcategories", rootCategoryId)
                        .param("activeOnly", "true")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    void shouldGetCategoryPathSuccessfully() throws Exception {
        ...
        when(categoryService.getCategoryPath(subCategoryId)).thenReturn(Arrays.asList(rootCategory, subCategory));
        when(categoryService.countProductsInCategory(any(UUID.class), anyBoolean())).thenReturn(1L);

        // When & Then
        mockMvc.perform(get("/api/categories/{id}/path", subCategoryId)
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(2)))
