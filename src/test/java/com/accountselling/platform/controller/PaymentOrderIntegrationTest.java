...
/**
 * Integration tests for Payment and Order workflows. Tests complete end-to-end functionality
 * including order creation, payment processing, and account download workflows with real database
 * and application context.
 *
 * <p>Integration tests สำหรับ Payment และ Order workflows ทดสอบฟังก์ชันการทำงานแบบ end-to-end
 * รวมถึงการสร้างออเดอร์ การประมวลผลการชำระเงิน และ workflow การดาวน์โหลดบัญชี พร้อมฐานข้อมูลจริงและ
 * application context
 */
@SpringBootTest
@AutoConfigureWebMvc
...
class PaymentOrderIntegrationTest {
  ...
  void completePaymentOrderWorkflow_Success() throws Exception {
    ...
    OrderCreateRequestDto orderRequest =
        new OrderCreateRequestDto(productQuantities, "Integration test order");
    ...
    PaymentCreateRequestDto paymentRequest =
        new PaymentCreateRequestDto(
            UUID.fromString(orderId), "QRCODE", 30, "Integration test payment");
    ...
    mockMvc
        .perform(get("/api/payments/status/{paymentId}", paymentId))
        .andDo(print())
    ...
    mockMvc
        .perform(get("/api/orders/{orderId}/download", orderId))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  void paymentFailureWorkflow_OrderRemainsUnchanged() throws Exception {
    ...
    productQuantities.put(testProduct.getId(), 1);
    OrderCreateRequestDto orderRequest =
        new OrderCreateRequestDto(productQuantities, "Failure test order");
    ...
    PaymentCreateRequestDto paymentRequest =
        new PaymentCreateRequestDto(UUID.fromString(orderId), "QRCODE", 30, "Failure test payment");
  ...
  void orderCancellationWorkflow_PaymentNotAllowed() throws Exception {
    ...
    productQuantities.put(testProduct.getId(), 1);
    OrderCreateRequestDto orderRequest =
        new OrderCreateRequestDto(productQuantities, "Cancellation test order");
    ...
    mockMvc
        .perform(
            delete("/api/orders/{orderId}", orderId).with(csrf()).param("reason", "Changed mind"))
        .andDo(print())
        .andExpect(status().isOk())
    ...
    PaymentCreateRequestDto paymentRequest =
        new PaymentCreateRequestDto(
            UUID.fromString(orderId), "QRCODE", 30, "Payment for cancelled order");
  ...
  void unauthorizedAccessWorkflow_AllEndpointsProtected() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/orders").with(csrf()).contentType(MediaType.APPLICATION_JSON).content("{}"))
        .andDo(print())
        .andExpect(status().isUnauthorized());

    mockMvc.perform(get("/api/orders")).andDo(print()).andExpect(status().isUnauthorized());
    ...
    mockMvc
        .perform(delete("/api/orders/{orderId}", UUID.randomUUID()))
        .andDo(print())
        .andExpect(status().isUnauthorized());

    mockMvc
        .perform(get("/api/orders/{orderId}/download", UUID.randomUUID()))
        .andDo(print())
        .andExpect(status().isUnauthorized());
    ...
    mockMvc
        .perform(
            post("/api/payments/generate").contentType(MediaType.APPLICATION_JSON).content("{}"))
        .andDo(print())
        .andExpect(status().isUnauthorized());

    mockMvc
        .perform(get("/api/payments/status/{paymentId}", UUID.randomUUID()))
        .andDo(print())
        .andExpect(status().isUnauthorized());
    ...
    mockMvc
        ...
        .andDo(print())
        .andExpect(status().isInternalServerError()); // Webhook is public but throws
    // WebhookProcessingException for invalid data
  }
  ...
  void crossUserAccessPrevention_UsersIsolated() throws Exception {
    ...
    mockMvc
        .perform(get("/api/orders/{orderId}", anotherOrder.getId()))
        .andDo(print())
        .andExpect(status().isForbidden());

    // Attempt to access another user's payment - Should Fail
    mockMvc
        .perform(get("/api/payments/status/{paymentId}", anotherPayment.getId()))
        .andDo(print())
        .andExpect(status().isBadRequest());

    // Attempt to download another user's order - Should Fail
    mockMvc
        .perform(get("/api/orders/{orderId}/download", anotherOrder.getId()))
        .andDo(print())
        .andExpect(
            status()
                .isForbidden()); // Service throws InsufficientPermissionsException for unauthorized
    // access
  }
}
