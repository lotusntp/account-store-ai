 ...
 * Tests complete functionality with real database and application context.
 * Rate limiting is disabled for these tests using TestRateLimitConfig.
 * 
 * Integration tests สำหรับ ProductController
 * ทดสอบฟังก์ชันการทำงานแบบครบถ้วนด้วยฐานข้อมูลจริงและ application context
 * มีการปิด rate limiting สำหรับ tests เหล่านี้โดยใช้ TestRateLimitConfig
 */
@SpringBootTest
@AutoConfigureWebMvc
@ActiveProfiles("test")
...
class ProductControllerIntegrationTest {
    ...
    void shouldGetAllActiveProductsSuccessfully() throws Exception {
        mockMvc.perform(get("/api/products")
                ...
                .andExpect(jsonPath("$.content[*].name", hasItems("World of Warcraft Account", "CS:GO Prime Account")))
    ...
    void shouldGetAllProductsIncludingInactive() throws Exception {
        mockMvc.perform(get("/api/products")
                ...
                .andExpect(jsonPath("$.content[*].name", hasItems(
                        "World of Warcraft Account", "CS:GO Prime Account", "Discontinued Game Account")))
                .andExpect(jsonPath("$.content[?(@.name == 'Discontinued Game Account')].active", contains(false)));
    }
    ...
    void shouldGetProductByIdSuccessfully() throws Exception {
        mockMvc.perform(get("/api/products/{id}", wowProduct.getId())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(wowProduct.getId().toString())))
                .andExpect(jsonPath("$.name", is("World of Warcraft Account")))
                .andExpect(jsonPath("$.description", is("High-level WoW account with rare items and achievements")))
    ...
    void shouldGetProductsByCategorySuccessfully() throws Exception {
        mockMvc.perform(get("/api/products/category/{categoryId}", mmorpgCategory.getId())
    ...
    void shouldGetProductsByCategoryWithSubcategories() throws Exception {
        mockMvc.perform(get("/api/products/category/{categoryId}", gamingCategory.getId())
    ...
    void shouldReturnEmptyListWhenCategoryHasNoProducts() throws Exception {
        ...
        mockMvc.perform(get("/api/products/category/{categoryId}", emptyCategory.getId())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    void shouldReturn400WhenSearchHasInvalidPriceRange() throws Exception {
        mockMvc.perform(get("/api/products/search")
                ...
                .andExpect(jsonPath("$.message", containsString("Maximum price must be greater than or equal to minimum price")));
    }
    ...
    void shouldIncludeCorrectStockInformation() throws Exception {
        ...
        mockMvc.perform(get("/api/products/{id}", csgoProduct.getId())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
