...
class ProductControllerTest {
  ...
  void shouldGetAllActiveProductsSuccessfully() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.content[0].description", is("High-level WoW account with rare items")))
  ...
  void shouldGetProductsWithAvailableStockWhenInStockOnly() throws Exception {
    ...
    when(productService.findProductsWithAvailableStock(any(Pageable.class)))
  ...
  void shouldGetProductsByCategorySuccessfully() throws Exception {
    ...
    when(productService.findProductsByCategory(
            eq(categoryId), eq(false), eq(true), any(Pageable.class)))
        .thenReturn(productPage);
    when(productService.getProductStockInfo(productId)).thenReturn(stockInfo);
  ...
  void shouldGetProductsByCategoryWithSubcategories() throws Exception {
    ...
    when(productService.findProductsByCategory(
            eq(categoryId), eq(true), eq(true), any(Pageable.class)))
        .thenReturn(productPage);
    when(productService.getProductStockInfo(productId)).thenReturn(stockInfo);
  ...
  void shouldSearchProductsSuccessfully() throws Exception {
    ...
    when(productService.searchProducts(
            any(ProductService.ProductSearchCriteria.class), any(Pageable.class)))
        .thenReturn(productPage);
    when(productService.getProductStockInfo(productId)).thenReturn(stockInfo);
  ...
  void shouldReturn400ForInvalidPaginationParameters() throws Exception {
    ...
    mockMvc
        .perform(get("/api/products").param("size", "0").contentType(MediaType.APPLICATION_JSON))
        .andDo(print())
        .andExpect(status().isBadRequest());
  }
