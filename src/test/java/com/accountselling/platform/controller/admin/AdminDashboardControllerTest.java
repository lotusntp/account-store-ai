...
/**
 * Comprehensive unit tests for AdminDashboardController.
 * Tests all dashboard operations, validation, authorization, and error handling.
 */
...
class AdminDashboardControllerTest {
    ...
    void getDashboardOverview_Success() throws Exception {
        ...
        when(productService.findAllProducts(any())).thenReturn(new org.springframework.data.domain.PageImpl<>(allProducts, org.springframework.data.domain.PageRequest.of(0, 1), 500L));
        ...
        verify(orderService, times(6)).getOrderStatistics(any(LocalDateTime.class), any(LocalDateTime.class));
    ...
    void getOrderStats_OnlyStartDate() throws Exception {
        ...
        verify(orderService).getOrderStatistics();
        verify(orderService, never()).getOrderStatistics(any(LocalDateTime.class), any(LocalDateTime.class));
    }
    ...
    void getDailyTrends_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/dashboard/trends/daily")
                .param("startDate", startDate.toString())
                .param("endDate", endDate.toString()))
                .andExpect(status().isOk())
    ...
    void getSystemHealth_Success() throws Exception {
        // Act & Assert
        mockMvc.perform(get("/api/admin/dashboard/system/health"))
                .andExpect(status().isOk())
    ...
    void getLowStockAlerts_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/dashboard/alerts/low-stock"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
    ...
    void getDashboardOverview_ServiceException() throws Exception {
        // Arrange
        when(userService.getTotalUserCount()).thenThrow(new RuntimeException("Database connection failed"));
    ...
    void getOrderStats_EmptyStatistics() throws Exception {
        // Arrange
        OrderStatistics emptyStats = new OrderStatistics(
                0L, // total
                0L, // pending
                0L, // processing
                0L, // completed
    ...
    void getDailyTrends_LargeDateRange() throws Exception {
        ...
        List<DailyOrderStatistics> largeTrendList = Arrays.asList(
                testDailyStatistics, testDailyStatistics, testDailyStatistics
        );
    ...
    void dashboardOverview_IncludesAllSections() throws Exception {
        ...
        when(orderService.getOrderStatistics(any(LocalDateTime.class), any(LocalDateTime.class)))
                .thenReturn(testOrderStatistics);
        when(productService.findAllProducts(any())).thenReturn(new org.springframework.data.domain.PageImpl<>(Arrays.asList(testProduct), org.springframework.data.domain.PageRequest.of(0, 1), 500L));
        when(categoryService.findAllCategories()).thenReturn(Arrays.asList(testCategory));
        when(categoryService.findActiveCategories()).thenReturn(Arrays.asList(testCategory));
