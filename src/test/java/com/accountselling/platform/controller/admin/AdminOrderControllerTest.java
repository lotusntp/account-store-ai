...
/**
 * Comprehensive unit tests for AdminOrderController.
 * Tests all order management operations, validation, authorization, and error handling.
 */
...
class AdminOrderControllerTest {
    ...
    void setUp() {
        ...
        testOrderStatistics = new OrderStatistics(
                100L, // total
                20L,  // pending
                10L,  // processing
                70L,  // completed
        ...
        testDailyStatistics = new DailyOrderStatistics(
    ...
    void searchOrders_InvalidPagination() throws Exception {
        ...
        verify(orderService, never()).searchOrders(any(), any(), any(), any(), any(), any(), any(), any());
    }
    ...
    void getOrderById_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/orders/{id}", testOrder.getId()))
                .andExpect(status().isOk())
    ...
    void getOrderById_NotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/orders/{id}", nonExistentId))
    ...
    void getOrdersByUser_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/orders/user/{userId}", testUser.getId())
                .param("page", "0")
                .param("size", "20"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(1)))
    ...
    void getOrdersByUser_UserNotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/orders/user/{userId}", nonExistentUserId))
                .andExpect(status().isNotFound());
    ...
    void getOrdersByStatus_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/orders/status/{status}", "PENDING")
                .param("page", "0")
                .param("size", "20"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(1)))
    ...
    void markOrderAsProcessing_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/orders/{id}/processing", testOrder.getId())
                .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(testOrder.getId().toString()))
    ...
    void markOrderAsProcessing_OrderNotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/orders/{id}/processing", nonExistentId)
                .with(csrf()))
                .andExpect(status().isNotFound());
    ...
    void markOrderAsCompleted_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/orders/{id}/completed", testOrder.getId())
                .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(testOrder.getId().toString()))
    ...
    void markOrderAsFailed_Success() throws Exception {
        ...
        when(orderService.markOrderAsFailed(testOrder.getId(), "Payment failed")).thenReturn(failedOrder);

        // Act & Assert
        mockMvc.perform(put("/api/admin/orders/{id}/failed", testOrder.getId())
                .with(csrf())
    ...
    void markOrderAsFailed_DefaultReason() throws Exception {
        ...
        when(orderService.markOrderAsFailed(testOrder.getId(), "Order marked as failed by admin")).thenReturn(failedOrder);
    ...
    void cancelOrder_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/orders/{id}/cancel", testOrder.getId())
                .with(csrf())
    ...
    void getDailyOrderStatistics_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/orders/statistics/daily")
                .param("startDate", startDate.toString())
                .param("endDate", endDate.toString()))
                .andExpect(status().isOk())
        ...
        verify(orderService).getDailyOrderStatistics(any(LocalDateTime.class), any(LocalDateTime.class));
    }
    ...
    void getTopCustomers_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/orders/top-customers")
                .param("limit", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
