...
/**
 * Comprehensive unit tests for AdminProductController.
...
class AdminProductControllerTest {
    ...
    void setUp() {
        ...
        // Setup stock info
        stockInfo = new ProductService.ProductStockInfo(
                testProduct.getId(),
                10L, // totalStock
                8L,  // availableStock
                2L,  // soldStock
        ...
        // Setup DTOs
        createRequest = ProductCreateRequestDto.builder()
                .name("New Product")
    ...
    void createProduct_InvalidInput() throws Exception {
        // Arrange
        ProductCreateRequestDto invalidRequest = ProductCreateRequestDto.builder()
        ...
        verify(productService, never()).createProduct(any(), any(), any(), any(), any(), any(), any(), any());
    }
    ...
    void createProduct_CategoryNotFound() throws Exception {
        ...
        when(productService.createProduct(any(), any(), any(), any(), any(), any(), any(), any()))
                .thenThrow(new ResourceNotFoundException("Category", createRequest.getCategoryId().toString()));
    ...
    void updateProduct_NotFound() throws Exception {
        // Arrange
        UUID nonExistentId = UUID.randomUUID();
        when(productService.updateProduct(nonExistentId, updateRequest.getName(), updateRequest.getDescription(), 
                updateRequest.getPrice(), updateRequest.getCategoryId(), updateRequest.getServer(), updateRequest.getImageUrl()))
                .thenThrow(new ResourceNotFoundException("Product", nonExistentId.toString()));

        // Act & Assert
        mockMvc.perform(put("/api/admin/products/{id}", nonExistentId)
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updateRequest)))
                .andExpect(status().isNotFound());

        verify(productService).updateProduct(eq(nonExistentId), any(), any(), any(), any(), any(), any());
    }
    ...
    void moveProductToCategory_Success() throws Exception {
        ...
        when(productService.moveProductToCategory(testProduct.getId(), newCategoryId)).thenReturn(movedProduct);
        when(productService.getProductStockInfo(testProduct.getId())).thenReturn(stockInfo);

        // Act & Assert
        mockMvc.perform(put("/api/admin/products/{id}/category", testProduct.getId())
                .with(csrf())
    ...
    void deleteProduct_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(delete("/api/admin/products/{id}", testProduct.getId())
                .with(csrf()))
    ...
    void deleteProduct_NotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(delete("/api/admin/products/{id}", nonExistentId)
                .with(csrf()))
                .andExpect(status().isNotFound());
    ...
    void deleteProduct_HasPendingOrders() throws Exception {
        ...
        doThrow(new IllegalStateException("Product has pending orders or stock"))
                .when(productService).deleteProduct(testProduct.getId());
    ...
    void forceDeleteProduct_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(delete("/api/admin/products/{id}/force", testProduct.getId())
                .with(csrf()))
                .andExpect(status().isNoContent());
    ...
    void getProductsByCategory_Success() throws Exception {
        ...
        when(productService.findProductsByCategory(eq(testCategory.getId()), eq(false), eq(false), any(Pageable.class)))
                .thenReturn(productPage);
        when(productService.getProductStockInfo(testProduct.getId())).thenReturn(stockInfo);

        // Act & Assert
        mockMvc.perform(get("/api/admin/products/category/{categoryId}", testCategory.getId())
        ...
        verify(productService).findProductsByCategory(eq(testCategory.getId()), eq(false), eq(false), any(Pageable.class));
    }
    ...
    void getAvailableServers_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/products/servers"))
                .andExpect(status().isOk())
