...
/**
 * Comprehensive unit tests for AdminStockController.
 * Tests all stock management operations, validation, authorization, and error handling.
 */
...
class AdminStockControllerTest {
    ...
    void setUp() {
        ...
        stockStatistics = new StockStatistics(
                100L, // total
                85L,  // available
                10L,  // sold
                5L    // reserved
    ...
    void getStockByProduct_Success() throws Exception {
        ...
        when(stockService.getStockByProduct(eq(testProduct.getId()), any(Pageable.class))).thenReturn(stockPage);

        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/product/{productId}", testProduct.getId())
                .param("page", "0")
                .param("size", "20"))
                ...
                .andExpect(jsonPath("$.content[0].additionalInfo").value("Level 80 character with rare items"))
    ...
    void getStockByProduct_ProductNotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/product/{productId}", nonExistentId))
    ...
    void getStockById_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/{id}", testStock.getId()))
                .andExpect(status().isOk())
    ...
    void getStockById_NotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/{id}", nonExistentId))
                .andExpect(status().isNotFound());
    ...
    void createStock_Success() throws Exception {
        ...
        verify(stockService).createStock(
                createRequest.getProductId(),
                createRequest.getCredentials(),
                createRequest.getAdditionalInfo()
        );
    ...
    void createStock_ProductNotFound() throws Exception {
        ...
        when(stockService.createStock(any(), any(), any()))
                .thenThrow(new ResourceNotFoundException("Product", createRequest.getProductId().toString()));
    ...
    void createBulkStock_Success() throws Exception {
        ...
        when(stockService.createBulkStock(
                bulkCreateRequest.getProductId(),
                bulkCreateRequest.getCredentialsList()
        )).thenReturn(createdStocks);

        // Act & Assert
        mockMvc.perform(post("/api/admin/stock/bulk")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(stockService).createBulkStock(
                bulkCreateRequest.getProductId(),
                bulkCreateRequest.getCredentialsList()
        );
    }
    ...
    void updateStock_Success() throws Exception {
        ...
        when(stockService.updateStockAdditionalInfo(testStock.getId(), updateRequest.getAdditionalInfo()))
                .thenReturn(updatedStock);

        // Act & Assert
        mockMvc.perform(put("/api/admin/stock/{id}", testStock.getId())
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(stockService).updateStockAdditionalInfo(testStock.getId(), updateRequest.getAdditionalInfo());
    }
    ...
    void updateStock_NotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/stock/{id}", nonExistentId)
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updateRequest)))
                .andExpect(status().isNotFound());

        verify(stockService).updateStockAdditionalInfo(nonExistentId, updateRequest.getAdditionalInfo());
    }
    ...
    void markAsSold_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/stock/{id}/sold", testStock.getId())
                .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(testStock.getId().toString()))
    ...
    void releaseReservation_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/stock/{id}/release", testStock.getId())
                .with(csrf()))
                .andExpect(status().isOk());
    ...
    void deleteStock_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(delete("/api/admin/stock/{id}", testStock.getId())
                .with(csrf()))
    ...
    void deleteStock_NotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(delete("/api/admin/stock/{id}", nonExistentId)
                .with(csrf()))
                .andExpect(status().isNotFound());
    ...
    void deleteStock_CannotDeleteSoldStock() throws Exception {
        ...
        doThrow(new IllegalStateException("Stock item is sold or reserved"))
                .when(stockService).deleteStock(testStock.getId());
    ...
    void getStockStatistics_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/product/{productId}/statistics", testProduct.getId()))
                .andExpect(status().isOk())
    ...
    void getProductsWithLowStock_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/low-stock"))
                .andExpect(status().isOk())
    ...
    void getExpiredReservations_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/expired-reservations"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].id").value(expiredStock.getId().toString()))
                .andExpect(jsonPath("$[0].status").value("AVAILABLE")); // Expired reservation should show as available
    ...
    void cleanupExpiredReservations_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(post("/api/admin/stock/cleanup-expired")
                .with(csrf()))
                .andExpect(status().isOk())
    ...
    void getSoldStock_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/stock/sold"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
    ...
    void getSoldStock_WithProductFilter() throws Exception {
        ...
        List<Stock> soldStockList = Arrays.asList(soldStock);
        when(stockService.getSoldStock(eq(testProduct.getId()), any(), any())).thenReturn(soldStockList);
