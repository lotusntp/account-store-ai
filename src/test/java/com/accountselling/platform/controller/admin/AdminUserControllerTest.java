...
/**
 * Comprehensive unit tests for AdminUserController.
 * Tests all user management operations, validation, authorization, and error handling.
 */
...
class AdminUserControllerTest {
    ...
    void getUserByUsername_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/admin/users/username/{username}", "johndoe"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(testUser.getId().toString()))
                .andExpect(jsonPath("$.username").value("johndoe"))
                .andExpect(jsonPath("$.email").value("john.doe@example.com"));
    ...
    void updateUser_UserNotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/users/{id}", nonExistentId)
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updateRequest)))
                .andExpect(status().isNotFound());
    ...
    void addRoleToUser_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/users/{id}/roles/{roleName}", testUser.getId(), "ADMIN")
                .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(testUser.getId().toString()))
    ...
    void addRoleToUser_RoleNotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/users/{id}/roles/{roleName}", testUser.getId(), "NONEXISTENT")
                .with(csrf()))
                .andExpect(status().isNotFound());
    ...
    void removeRoleFromUser_Success() throws Exception {
        ...
        when(userService.removeRoleFromUser(adminUser.getId(), "ADMIN")).thenReturn(userWithoutAdminRole);

        // Act & Assert
        mockMvc.perform(delete("/api/admin/users/{id}/roles/{roleName}", adminUser.getId(), "ADMIN")
                .with(csrf()))
                .andExpect(status().isOk())
    ...
    void resetUserPassword_Success() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/users/{id}/reset-password", testUser.getId())
                .with(csrf())
    ...
    void resetUserPassword_UserNotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/admin/users/{id}/reset-password", nonExistentId)
                .with(csrf())
                .param("newPassword", "newpassword123"))
                .andExpect(status().isNotFound());
