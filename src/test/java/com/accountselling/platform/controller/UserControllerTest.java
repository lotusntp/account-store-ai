 ...
 * Tests user profile management, order history, and account downloads.
 */
@SpringBootTest
@AutoConfigureWebMvc
@ActiveProfiles("test")
...
class UserControllerTest {
    ...
    void shouldDownloadAccountDataForCompletedOrder() throws Exception {
        ...
        mockMvc.perform(get("/api/users/download/" + order.getId())
                        .header("Authorization", "Bearer " + accessToken))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(header().string("Content-Type", "text/plain;charset=UTF-8"))
                .andExpect(header().string("Content-Disposition", 
                    containsString("attachment; filename=\"accounts_order_" + order.getId() + ".txt\"")))
    ...
    void shouldRejectDownloadForNonExistentOrder() throws Exception {
        ...
        mockMvc.perform(get("/api/users/download/" + nonExistentOrderId)
                        .header("Authorization", "Bearer " + accessToken))
                .andDo(print())
    ...
    void shouldRejectDownloadWithoutAuthentication() throws Exception {
        ...
        mockMvc.perform(get("/api/users/download/" + orderId))
                .andDo(print())
                .andExpect(status().isUnauthorized());
    }
